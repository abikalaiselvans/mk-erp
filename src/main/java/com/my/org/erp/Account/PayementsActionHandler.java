...
public class PayementsActionHandler extends AbstractActionHandler {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try 
		{
			...
			if ("ACCPayementsAdd".equals(action)) 
			{
				...
				sql="INSERT INTO  acc_t_payments  (INT_GROUPID,INT_COMPANYID,INT_BRANCHID,CHR_DESC,DAT_DATE,DOU_AMOUNT,INT_FINANCIALYEARID,CHR_USRNAME,DT_UPDATEDATE,CHR_UPDATESTATUS) VALUES ";
				sql = sql +"( ";
				sql = sql +" "+group+", ";
				sql = sql +" "+cpy+", ";
				sql = sql +" "+bch+", ";
				...
				st.execute(sql);
				st.close();
				response.sendRedirect("Smart Accounts/Payements.jsp");
			}
			else if ("ACCPayementsEdit".equals(action)) 
			{
			...
			else if ("ACCPayementsDelete".equals(action)) 
			{
				String ids[] =request.getParameterValues("expenseid");
				for(int u=0;u<ids.length;u++)
					st.execute(" DELETE FROM  acc_t_payments  WHERE INT_EXPENSEID="+ids[u]);
				st.close();
				response.sendRedirect("Smart Accounts/Payements.jsp");
			}
			st.close();
