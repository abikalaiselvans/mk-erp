package com.my.org.erp.ResignedStaff;
...
public class ExitInterviewActionHandler extends AbstractActionHandler 
{
	...
	public void handle(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException 
	{
		try 
		{
			...
			if("RESExitInterviewAdds".equals(action)) 
			{
				...
				con.close();   
				response.sendRedirect("Resigned/Userframe.jsp");
			} 
	...
	private void ExitInterviewAdds(HttpServletRequest request,HttpServletResponse response,ExitInterview d) throws ServletException, IOException 
	{
		...
		catch (Exception e) 
		{
			System.out.println(e.getMessage());
			request.setAttribute("error", e.getMessage() );
            RequestDispatcher dispatchers = request.getRequestDispatcher("/Staff/error.jsp");
    		dispatchers.forward(request, response); 
