	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	 {
		try
		{
				...
				if("AMCServiceCustomerInfoAdd".equals(action))
				{
					...
					acs.execute();
					 //9884533777
					con.close();   
					response.sendRedirect("Smart AMC/ServiceCustomerInfo.jsp");
				}	
				else if("AMCServiceCustomerInfoUpdate".equals(action))
				{
					String group=request.getParameter("group");
				...
				else if("AMCServiceCustomerInfoDelete".equals(action))
				{
