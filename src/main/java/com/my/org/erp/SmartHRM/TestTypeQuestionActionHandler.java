...
public class TestTypeQuestionActionHandler extends AbstractActionHandler 
{
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		try 
		{
			...
			if (action.equals("HRMTestTypeQuestionDelete")) 
			{
				String ids[] = request.getParameterValues("id");
				String type=request.getParameter("type");
				acs = con.prepareCall("{call  HRM_PRO_TESTTYPEQUESTION(?,?,?,?,?,?,?,?,?,? )}");
				...
				response.sendRedirect("Smart HRM/TestTypeQuestion.jsp?type="+type);
			}
			else if ( "HRMTestTypeQuestionADD".equals(action) ) 
			{
				String type=request.getParameter("type");
			...
			else if (action.equals("HRMTestTypeQuestionEdit")) 
			{
