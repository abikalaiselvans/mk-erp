	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	 {
		try
		{
				...
				if("INVServiceItemAdd".equals(action))
				{
					...
					st.execute(sql);
					con.close();    
					response.sendRedirect("Smart Inventory/ServiceItems.jsp");
				}	
				else if("INVServiceItemEdit".equals(action))
				{
					String ServiceItem=request.getParameter("ServiceItem");
				...
				else if("INVServiceItemDelete".equals(action))
				{
