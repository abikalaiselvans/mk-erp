...
public class NewSalaryProcessActionHandler extends AbstractActionHandler 
{ 
					...
					if(flag)
	 				{
							...
							for(int v=0;v<attdata.length;v++)
							{
								try 
								{
									...
									originalbasic = basic;
									basic= basic-((basic*12/(365+extradays))*absent); 
									...
									{	
										for(int f=0;f<aname.length;f++)
										{
											...
											{	
												...
												addedallowancesal=addedallowancesal-((addedallowancesal*12/(365+extradays))*absent);
									...
									{	
										...
										{
											pfValue = ( dbminimumpflimit-((dbminimumpflimit*12/(365+extradays))*absent) ) * pfValue / 100;
											CompanyPF1=(double)( dbminimumpflimit-((dbminimumpflimit*12/(365+extradays))*absent) )*(cpypension/100);
											CompanyPF2=(double)( dbminimumpflimit-((dbminimumpflimit*12/(365+extradays))*absent) )*(cpypf/100);
									...
									if(!pftype.equals("N"))
									{
										if(originalbasic<dbminimumpflimit)
										{
											...
											penbasic =( dbminimumpflimit-( (dbminimumpflimit*12/(365+extradays))*absent));
											pension = (( dbminimumpflimit-((dbminimumpflimit*12/(365+extradays))*absent) )  *cpypension/100);
										}
										else
										{
											if(basic>dbpflimit)
