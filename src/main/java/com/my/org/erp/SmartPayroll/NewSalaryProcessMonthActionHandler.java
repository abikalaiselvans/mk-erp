...
public class NewSalaryProcessMonthActionHandler extends AbstractActionHandler 
{ 
					...
					if(flag)
	 				{
							...
							String attempids="";
							/*for(int v=0;v<attdata.length;v++)
								attempids =attempids +"'"+attdata[v][0].trim()+"' , ";
							asql = "DELETE FROM pay_t_salary WHERE  INT_YEAR=? AND CHR_MONTH=? AND CHR_EMPID IN(?) ";
							apstm = con.prepareStatement(asql);
							apstm.setString(1,year);
							apstm.setString(2,mon);
							apstm.setString(3,empid+" '0' ");
							System.out.println("DELETE THE EMPLOYEE IDS : <br>"+apstm);
							apstm.execute();
							apstm.close();*/
							...
							for(int v=0;v<attdata.length;v++)
							{
								try 
								{
									...
									noofdaysinamonth = noofdaysinamonth + extradays;
									...
									System.out.println(dbminimumpflimit+"/"+dbpflimit+"/"+dbesilimit); 
									System.out.println(pftype+ (pftype.equals("P"))); 
									if (pftype.equals("P") || pftype.equals("C") )
									{	
										//Orginal basic less than 3200 then pf calculated by 3200
										System.out.println(empid+"/"+originalbasic+"/"+dbminimumpflimit +"/"+(originalbasic<dbminimumpflimit));
										if(originalbasic<dbminimumpflimit)
										{
											pfValue = ( dbminimumpflimit-((dbminimumpflimit/noofdaysinamonth)*absent) ) * pfValue / 100;
											CompanyPF1=(double)( dbminimumpflimit-((dbminimumpflimit/noofdaysinamonth)*absent) )*(cpypension/100);
											CompanyPF2=(double)( dbminimumpflimit-((dbminimumpflimit/noofdaysinamonth)*absent) )*(cpypf/100);
										}
										else
										{
											if(basic<dbpflimit)
									...
									//ESI
									if (esitype.equals("P")|| esitype.equals("C"))
									{	
									...
									System.out.println("Salary Emp id : " +empid +" / "+ v+" @ "+ attdata.length +"/"+pfValue+"/"+CompanyPF1);
