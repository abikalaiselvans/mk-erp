...
public class PayrollMethodmsl extends HttpServlet {
   ...
   public static void PAYExit(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
      try {
         ...
         sql = sql + " where A.INT_BRANCHID=B.INT_BRANCHID AND A.INT_COMPANYID =E.INT_COMPANYID ";
         sql = sql + " AND A.INT_DEPARTID=C.INT_DEPARTID AND A.INT_DESIGID=D.INT_DESIGID " + " AND A.CHR_EMPID=F.CHR_EMPID  " + " AND year(F.DAT_CREATIONTIME)=" + year + " ";
         if (!"0".equals(month)) {
   ...
   public static void PAYResignation(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
      try {
         ...
         sql = sql + " where A.INT_BRANCHID=B.INT_BRANCHID AND A.INT_COMPANYID =E.INT_COMPANYID ";
         sql = sql + " AND A.INT_DEPARTID=C.INT_DEPARTID AND A.INT_DESIGID=D.INT_DESIGID " + " AND A.CHR_EMPID=F.CHR_EMPID  AND month(F.DAT_APPPLYDATE)=" + month + "  ";
      ...
      try {
         ...
         if (data.length > 0) {
            ...
            if (recoverydata.length > 0) {
               for(u = 0; u < recoverydata.length; ++u) {
