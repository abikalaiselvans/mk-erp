...
public class SalaryProcessLucasActionHandler extends AbstractActionHandler 
{ 
	...
	public synchronized void handle(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException 
	{
		try 
		{
 			...
 			if ("PAYSalary".equals(action)) 
			{
				...
				boolean  attflag = AttendanceFunctions.attendanceRegisterCheck(con, Integer.parseInt(month), year, attendanceData);
				if(attflag)
				{
					if(flag)
	 				{
						...
						noofdaysinamonth=Integer.parseInt(CommonFunctions.QueryExecute(" SELECT day(last_day('"+year+"-"+month+"-12' )) ")[0][0]);
		 				...
		 				noofdaysinamonth = noofworkingdaysinamonth;
						...
						for(int v=0;v<attdata.length;v++)
						{
							...
							originalbasic = basic;
							basic= (basic/noofdaysinamonth)*totaldays; 
							...
							{	
								for(int f=0;f<aname.length;f++)
								{
									...
									{	
										...
										addedallowancesal=(addedallowancesal/noofdaysinamonth)*totaldays;
							...
							{	
								...
								{	
									...
									{
										pfValue = ( (dbminimumpflimit/noofdaysinamonth)*totaldays ) * pfValue / 100;
										CompanyPF1=(double)( (dbminimumpflimit/noofdaysinamonth)*totaldays )*(cpypension/100);
										CompanyPF2=(double)( (dbminimumpflimit/noofdaysinamonth)*totaldays )*(cpypf/100);
									}
									else
									{
										if(basic<dbpflimit)
							...
							if(!pftype.equals("N"))
							{
								...
								if(originalbasic<dbminimumpflimit)
								{
									...
									penbasic=(  (dbminimumpflimit/noofdaysinamonth)* totaldays);
									pension=(( (dbminimumpflimit/noofdaysinamonth)*totaldays )  *cpypension/100);
