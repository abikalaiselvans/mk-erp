...
public class SalaryProcessWorkActionHandler extends AbstractActionHandler {
	...
	public synchronized void handle(HttpServletRequest request,
		...
		try {
			...
			if ("PAYSalary".equals(action)) {
				...
				sql = " SELECT COUNT(*) ";
				sql = sql + " FROM " + attendanceTable
						+ " a ,com_m_staff b , pay_t_salarybasic c";
				sql = sql + " WHERE a.DOU_TOTALDAYS >0 AND a.CHR_MONTH='" + mon
						+ "' ";
				sql = sql + " AND a.INT_YEAR= " + year;
				sql = sql
						+ " AND a.CHR_EMPID=b.CHR_EMPID AND a.CHR_EMPID=c.CHR_EMPID  ";
				...
				sql = sql + " AND b.DT_DOJCOLLEGE<='" + year + "-" + month
						+ "-31" + "'";
				if (!"0".equals(dept))
				...
				if(attflag)
				{
					...
					if (flag) 
					{
						...
						noofdaysinamonth = 0;
						noofsundaysinamonth = 0;
						noofworkingdaysinamonth = 0;
						...
						for (int v = 0; v < attdata.length; v++) 
						{
							...
							// ADVANCE
							if (CommonFunctions.RecordExist(con,
									"SELECT Count(*)  FROM pay_t_advance WHERE CHR_EMPID='"
											+ empid
											+ "' AND CHR_TYPE='Y' AND CHR_MONTH='"
											+ mon + "' AND INT_YEAR=" + year)) {
								String advsql = "SELECT  INT_DUEAMT  FROM pay_t_advance WHERE CHR_EMPID='"
										+ empid	+ "' AND CHR_TYPE='Y'  AND CHR_MONTH='"
										+ mon + "' AND INT_YEAR=" + year;
								String adv[][] = CommonFunctions.RecordSetArray(con, advsql);
								advance = Double.parseDouble(adv[0][0]);
							}
	
							// loss of pay calculations
							originalbasic = basic;
							//basic = basic- ((basic / noofworkingdaysinamonth) * absent);
							...
							sqlAllTran = sqlAllTran + " WHERE CHR_EMPID='" + empid
									+ "' AND CHR_MONTH='" + mon + "' AND INT_YEAR="
									+ year;
							...
							if (CommonFunctions.RecordExist(con, ssql)) 
							{
								for (int f = 0; f < aname.length; f++) 
								{
									if ("Y".equals(aname[f][1])) 
									{
										...
										//addedallowancesal = addedallowancesal- ((addedallowancesal / noofworkingdaysinamonth) * absent);
							...
							String sqlrvalues = "";
							String rvalue[][] =CommonFunctions
									.RecordSetArray(con, sqlRecTran);
							...
							if (!pftype.equals("N")) 
							{
								if (pftype.equals("P")) 
								{
									...
									// Orginal basic less than 3200 then pf
									// calculated by 3200
									if (originalbasic < dbminimumpflimit) 
									{
										...
										CompanyPF1 = (double)  (dbminimumpflimit / noofworkingdaysinamonth  * totaldays )* (cpypension / 100);
										CompanyPF2 = (double) (dbminimumpflimit / noofworkingdaysinamonth  * totaldays )* (cpypf / 100);
										
										//pfValue = (dbminimumpflimit - ((dbminimumpflimit / noofworkingdaysinamonth) * absent))	* pfValue / 100;
										//CompanyPF1 = (double) (dbminimumpflimit - ((dbminimumpflimit / noofworkingdaysinamonth) * absent))* (cpypension / 100);
										//CompanyPF2 = (double) (dbminimumpflimit - ((dbminimumpflimit / noofworkingdaysinamonth) * absent))* (cpypf / 100);
									} 
									else 
									{
										if (basic < dbpflimit) 
							...
							if (!pftype.equals("N")) 
							{
								...
								if (originalbasic < dbminimumpflimit) 
								{
									penbasic = dbminimumpflimit;// 3200 changed to
									...
									//penbasic = (dbminimumpflimit - ((dbminimumpflimit / noofworkingdaysinamonth) * absent));
									//pension = ((dbminimumpflimit - ((dbminimumpflimit / noofworkingdaysinamonth) * absent))* cpypension / 100);
							...
							// Pay Tax Calculation
							sql = "";
							sql = "SELECT COUNT(*) FROM pay_t_taxempids  WHERE CHR_EMPID='"
									+ empid
									+ "' AND INT_YEAR="
									+ year
									+ " AND CHR_MONTH='" + mon + "' ";
								...
								if (CommonFunctions.RecordExist(con, sql))// If record true
									if ("Y".equals(compaytaxdeduction))
										for (int t = 0; t < taxdata.length; t++)
											if ((originalgross >= Integer
													.parseInt(taxdata[t][0]))
													&& (originalgross <= Integer
															.parseInt(taxdata[t][1])))
												if ("P".equals(compaytaxvalue))
													taxvalue = taxvalue
															+ ((originalgross) * (Double
																	.parseDouble(taxdata[t][2]) / 100));
												else
													taxvalue = taxvalue
															+ Double.parseDouble(taxdata[t][3]);
	
							} else {
								if (!CommonFunctions
										.RecordExist(con, sql))// If record true
									if ("Y".equals(compaytaxdeduction))
										for (int t = 0; t < taxdata.length; t++)
											if ((originalgross >= Integer
													.parseInt(taxdata[t][0]))
							...
							sql = "SELECT count(*) from com_m_staff WHERE CHR_EMPID='"
									+ empid
									+ "' AND year(DT_DOJCOLLEGE)="
									+ year
									+ " AND monthname(DT_DOJCOLLEGE)='" + mon + "'";
							...
							sql = "DELETE FROM pay_t_salary WHERE CHR_EMPID='"
									+ empid + "' AND INT_YEAR=" + year
									+ " AND CHR_MONTH='" + mon + "' ";
							...
							sql = sql + noofworkingdaysinamonth + " , ";
							sql = sql + totaldays + " , ";
							sql = sql + basic + " , ";
							sql = sql + sqlavalues;
							...
							sql = sql + mess + ",";
							sql = sql + advance + ",";
							totalrecovery = recvalue + pfValue + esiValue
									+ taxvalue;
	
							sql = sql + totalrecovery + ",";
							totalsal = (grosspay - (recvalue + pfValue + esiValue
									+ mess + advance + taxvalue));
							monthsal = ((grosspay - (recvalue + pfValue + esiValue
									+ mess + advance + taxvalue)) + extraallowance);
							ctcsal = (grosspay + CompanyPF1 + CompanyPF2 + CompanyESI);
							sql = sql + totalsal + ",";
							sql = sql + monthsal + ",";
							sql = sql + CompanyPF1 + ",";
							...
							sql = sql + taxvalue + ",";
							sql = sql + " null,'0',null,null,'" + loginempid
									+ "' ,'" + userid + "' , DATE(NOW()),'Y' ";
	
							sql = sql + ")";
	
							System.out.println("Emp id : " + empid + " / " + v
									+ " @ " + attdata.length);
							apstm = con.prepareStatement(sql);
							apstm.execute();
							apstm.close();
						...
						con.close();
						response.sendRedirect("Smart Payroll/Salary Report.jsp?er=Salary processed for "
								+ attdata.length + " employees ...");
					} 
					else 
					{
						con.close();
