...
public class  CustomerInfoActionHandler extends AbstractActionHandler {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (action.equals("SMSCustomerInfoDelete")) 
			{
				String ids[] = request.getParameterValues("id");
				CallableStatement cs = con.prepareCall("{call SMS_PRO_CUSTOMERINFO(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
				for (int i = 0; i < ids.length; i++) {
					...
					cs.addBatch();
					System.out.println("sms..."+cs);

				}
				cs.executeBatch();
				cs.close();
				 
				response.sendRedirect("Smart SMS/CustomerInfo.jsp");
			}
			else if (action.equals("SMSCustomerInfoAdd")) 
			{
			...
			else if (action.equals("SMSCustomerInfoEdit")) 
			{
