...
{
	public static void searchcomp(String file)
    {
            File[] drives=File.listRoots();
            for (int ctr=0;ctr<(drives.length-1);ctr++)
            {
                    //System.out.println(search(drives[ctr],file));
            }               
    }
    
    
    public static String search(File dir,String file)
    {       String s ="";
            try {
                ...
                for(int ctr=0;ctr<directories.length;ctr++)
                {
                        ...
                        else
                        {
                                if(directories[ctr].getName().equals(file))
                                {
                                        ...
                                        s= s+directories[ctr].getAbsolutePath()+",";
                                }
                        }
                }
                return s;
