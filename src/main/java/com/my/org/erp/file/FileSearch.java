...
class FileSearch extends JFrame implements ActionListener, ItemListener
{
	...
	public FileSearch()
	{
   	...
   	super("Nss1.2.5 File search");
		...
		con=getContentPane();
		scrollList.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.blue),"Search Result..."));
	...
	{
		if ((e.getSource().equals(searchButton)) && (!(searchField.getText().trim().equals(""))))
		{
		...
		else if ((e.getSource().equals(runButton)))
		{
			...
			System.out.println((String)(listModel.get(searchList.getSelectedIndex())));
		try {
		Runtime.getRuntime().exec("rundll32 SHELL32.DLL,ShellExec_RunDLL " + (String)(listModel.get(searchList.getSelectedIndex())));
			} catch (Exception exception) {
				System.out.println("An error occured trying to execute file.");
			}
		}
		else
		JOptionPane.showMessageDialog(this,"There is no selected list Item to run","Error in List item",JOptionPane.ERROR_MESSAGE);
	   }
	...
	private void search(File dir, String name, DefaultListModel model, JLabel lb)
	{
	  ...
	  if(dir.getName().toLowerCase().equals(name.toLowerCase()))
		...
		for(int i=0;i<children.length;i++){	search(new File(dir,children[i]),name,model,lb);}
