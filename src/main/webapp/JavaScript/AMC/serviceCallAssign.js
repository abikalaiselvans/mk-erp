...
function LoadAMCCallAssign(startLetter) 
{
	...
	var url = "../AMC?actionS=LoadAMCCallAssign&StartLetter="+escape(startLetter)+"&year="+escape(year)+"&month="+escape(month)+"&callstatus="+callstatus+"&servicecenter="+servicecenter+"&branch="+branch;
	initRequest(url);
...
function AMCCallAssignMessages() 
{
    	...
    	for(loop = 0; loop < batchs.childNodes.length; loop++) 
    	{
        	...
        	if((CallStatus.childNodes[0].nodeValue == "N") || (CallStatus.childNodes[0].nodeValue == "P"))
        		str = str+"<td class='boldEleven'><a href='ServiceCallAssignAdd.jsp?callid="+Bookingid.childNodes[0].nodeValue+"'><font color='green'><b>Call Assign</b></font></a>";
        	else
