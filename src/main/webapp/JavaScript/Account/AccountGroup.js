...
function AccountGroup(startLetter)
 {
 	var url = "../Account?actionS=LoadAccountGroup&startLetter="+escape(startLetter);
    initRequest(url);
	req.onreadystatechange=AccountGroupRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function AccountGroupRequest() {
    if(req.readyState == 4) {    	
        ...
        if (req.status == 200) 
        {		  	
          AccountGroupGroupMessages();
        }
    }
}
function AccountGroupGroupMessages() 
{	 
