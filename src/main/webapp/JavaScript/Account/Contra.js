...
function LoadContra(startLetter)
 {
 	var month = document.getElementById('month').value;
 	var year = document.getElementById('year').value;
 	var customer = document.getElementById('customer').value;
 	var url = "../Account?actionS=LoadContra&startLetter="+escape(startLetter)+"&month="+month+"&year="+year+"&customer="+customer;
   	initRequest(url);
	req.onreadystatechange=ContraRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function ContraRequest() {
    if(req.readyState == 4) {    	
