...
function LoadJournel(startLetter)
 {
 	var month = document.getElementById('month').value;
 	var year = document.getElementById('year').value;
 	var customer = document.getElementById('customer').value;
 	var url = "../Account?actionS=LoadJournel&startLetter="+escape(startLetter)+"&month="+month+"&year="+year+"&customer="+customer;
   	initRequest(url);
	req.onreadystatechange=JournelRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function JournelRequest() {
    if(req.readyState == 4) {    	
