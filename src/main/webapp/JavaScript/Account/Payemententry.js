...
function LoadPaymentsEntry(startLetter)
 {
 	var month = document.getElementById('month').value;
 	var year = document.getElementById('year').value;
 	var customer = document.getElementById('customer').value;
 	var url = "../Account?actionS=LoadPaymentsEntry&startLetter="+escape(startLetter)+"&month="+month+"&year="+year+"&customer="+customer;
   	initRequest(url);
	req.onreadystatechange=PaymentsEntryRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function PaymentsEntryRequest() {
    if(req.readyState == 4) {    	
        ...
        if (req.status == 200) 
        {		  	
          PaymentsEntryMessages();
        }
    }
}
function PaymentsEntryMessages() 
{	 
