...
function LoadReceiptsEntry(startLetter)
 {
 	var month = document.getElementById('month').value;
 	var year = document.getElementById('year').value;
 	var customer = document.getElementById('customer').value;
 	var url = "../Account?actionS=LoadReceiptsEntry&startLetter="+escape(startLetter)+"&month="+month+"&year="+year+"&customer="+customer;
   	initRequest(url);
	req.onreadystatechange=ReceiptsEntryRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function ReceiptsEntryRequest() {
    if(req.readyState == 4) {    	
        ...
        if (req.status == 200) 
        {		  	
          ReceiptsEntryMessages();
        }
    }
}
function ReceiptsEntryMessages() 
{	 
