...
function LoadSalesReturn(startLetter)
 {
 	...
 	var url = "../Account?actionS=LoadSalesReturn&startLetter="+escape(startLetter)+"&month="+month+"&year="+year+"&customer="+customer;
   	initRequest(url);
	req.onreadystatechange=SalesReturnsRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function SalesReturnsRequest() {
    if(req.readyState == 4) {    	
        ...
        if (req.status == 200) 
        {		  	
          SalesReturnsMessages();
        }
    }
}
function SalesReturnsMessages() 
{	 
    ...
    {	     
	    ...
	    var Amount = batch.getElementsByTagName("Amount")[0].childNodes[0].nodeValue;
	     var Sales = batch.getElementsByTagName("Sales")[0].childNodes[0].nodeValue;
		...
		str=str+"<td  class='boldEleven'>&nbsp;"+Sales;
