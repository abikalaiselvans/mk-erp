...
function LoadTypeGroup(startLetter)
 {
 	var type = document.getElementById('type').value;
 	var url = "../Account?actionS=LoadTypeGroup&startLetter="+escape(startLetter)+"&type="+type;
 	initRequest(url);
	req.onreadystatechange=TypeGroupRequest;
    req.open("GET", url, true);
    req.send(null);
}

 
 function TypeGroupRequest() {
    if(req.readyState == 4) {    	
        ClearTable('divTable');  
