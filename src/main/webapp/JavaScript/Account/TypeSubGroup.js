...
function LoadTypeSubGroup(startLetter)
 {
 	var group = document.getElementById('group').value;
 	var url = "../Account?actionS=LoadTypeSubGroup&startLetter="+escape(startLetter)+"&groups="+group;
 	initRequest(url);
	req.onreadystatechange=TypeSubGroupRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function TypeSubGroupRequest() {
    if(req.readyState == 4) {    	
        ClearTable('divTable');  
        ...
        if (req.status == 200) 
        {		  	
         TypeSubGroupMessages();
        }
    }
}

function TypeSubGroupMessages() 
{	 
