...
function LoadVouchers(startLetter)
 {
 	var url = "../Account?actionS=LoadVouchers&startLetter="+escape(startLetter);
   	initRequest(url);
	req.onreadystatechange=LoadVouchersRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function LoadVouchersRequest() 
 {
    if(req.readyState == 4) 
    {    	
        ...
        if (req.status == 200) 
        {		  	
          LoadVouchersMessages();
        }
    }
}
function LoadVouchersMessages() 
{	 
