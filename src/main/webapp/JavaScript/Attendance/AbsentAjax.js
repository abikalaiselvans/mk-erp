...
function LoadAbsentDays(startLetter) 
{		
    ...
    var url = "../attendance?actionS=LoadAbsentDays&month="+escape(mon)+"&year="+escape(yea)+"&startLetter="+escape(startLetter)+"&Office="+Office+"&day="+day+"&staffid="+staffid;
	initRequest(url);
	req.onreadystatechange=LoadAbsentDaysRequest;
    req.open("GET", url, true);
    req.send(null);
}


function LoadAbsentDaysRequest() 
{
    if(req.readyState == 4) 
    {    	
       
        ClearTable('AbsentTable');
    	ClearTabler('totRec');
		  		
        if (req.status == 200) 
        {		  	
          LoadAbsentDaysMessages();
        }
    }
}
...
function LoadAbsentDaysMessages() 
{	 
    ...
    {	     
		...
		str=str+"<td class='boldEleven'>&nbsp;Absent Entry</td>";
    }
     ...
     var tb=document.getElementById('AbsentTable');
  	 tb.innerHTML=str  
