...
function LoadEmailCustomerinfos(startLetter)
 {
 
 	var url = "../attendance?actionS=LoadEmailCustomerinfo&startLetter="+escape(startLetter)+"&group="+document.getElementById('group').value;
   	 
   	initRequest(url);
	req.onreadystatechange=EmailCustomerinfoRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function EmailCustomerinfoRequest() 
 {
    if(req.readyState == 4)
     {    	
        ...
        if (req.status == 200) 
        {		  	
          EmailCustomerinfoMessages();
        }
    }
}

function EmailCustomerinfoMessages() 
{	 
