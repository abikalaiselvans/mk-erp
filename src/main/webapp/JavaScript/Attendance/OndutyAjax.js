...
function ClearTabler(ctr1)
{
 ...
 function loadOnDuty(startLetter) 
{
    ...
    var url = "../attendance?actionS=AttOnduty&month="+escape(mon)+"&year="+escape(yea)+"&startLetter="+escape(startLetter)+"&Office="+office+"&day="+day+"&staffid="+staffid;
	initRequest(url);
	req.onreadystatechange = OnDutyRequest;
    req.open("GET", url, true);
    req.send(null);
}


function OnDutyRequest() 
{
    if(req.readyState == 4) 
    {    	
        ClearTable('OnDutyTable');
        ClearTabler('totRec');
        if (req.status== 200) 
        {		  	
          OnDutyMessages();
...
{	 
    ...
    str=str+"</table>";
     var tb=document.getElementById('OnDutyTable');
  	 tb.innerHTML=str  ; 
