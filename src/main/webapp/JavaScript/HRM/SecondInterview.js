...
function loadSecondInterview (startLetter) 
{    
	...
	var url = "../SmartHRM?actionS=loadSecondInterview&mon="+escape(mon)+"&yr="+escape(yr)+"&startLetter="+escape(startLetter)+"&jobtitle="+jobtitle+"&jobgrade="+jobgrade+"&day="+day+"&office="+office; 
    
   	initRequest(url);
    req.onreadystatechange = loadSecondInterviewRequest;
    req.open("GET", url, true);
    req.send(null);
}
function loadSecondInterviewRequest() 
{
    if(req.readyState == 4) {    
...
function loadSecondInterviewDetails() 
{
	...
	str=str+"<td class='bold1'  >1st Int By</td>";  
	...
	for(loop = 0; loop < batchs.childNodes.length; loop++) 
    {
       ...
       if(loop%2==1)
	    ...
	    /*
		if((SInterviewedstatus == "N") || (finalstatus =="Y") )
			str=str+"<td align='left' class='boldEleven'>"+Name+"  </td>"; 
		else
			str=str+"<td align='left' class='boldEleven'><a href='HRSecondInterviewUpdate.jsp?rowid="+Id+"'><font class='boldgreen'>"+Name+" </font></a> </td>";  
		...
		*/
		...
		if("Y" == SInterviewedstatus)
			str=str+"<td align='left' class='boldEleven'><font class='boldgreen'>Pass</font></td>";
		else if(  "F" == SInterviewedstatus)
			str=str+"<td align='left' class='boldEleven'><font class='bolddeepred'>Fail</font></td>";
		else
			str=str+"<td align='left' class='boldEleven'><font class='bolddeepred'>Not Attend</font></td>";
		
		 
		if(  "N" != SInterviewedstatus)
			str=str+"<td align='left' class='boldEleven'><a target='_blank' href='SecondInterviewPrint.jsp?rowid="+Id+"'><font class='boldgreen'>Print</font></a>  </td>"; 
		else
