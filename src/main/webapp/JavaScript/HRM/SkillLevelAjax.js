...
function loadSkill(startletter)
 {
 	var url = "../SmartHRM?actionS=loadSkill&search="+startletter ;
	 
	initRequest(url);
	req.onreadystatechange=loadSkillRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function loadSkillRequest() 
 {
    if(req.readyState == 4) 
    {    	
        ClearTable('loadSkillTable');  
        ClearTabler('totalRec');  
		 
		if (req.status == 200) 
        {		  	
         	var tb=document.getElementById('loadSkillTable');
...
{	 
    ...
    {	     
		...
		//str=str+"<td  class='boldEleven'><a href='SkillLevelAction.jsp?id="+ID+"'><font class='boldgreen'>"+Name+"</font></a></td>";
