...
function LoadAccessoriesProduct(startLetter)
 {
 	var group = document.getElementById('Group').value;
 	var url = "../inventory?actionS=LoadAccessoriesProduct&startLetter="+escape(startLetter)+"&group="+group;;
   	initRequest(url);
	req.onreadystatechange=AccessoriesProductRequest;
    req.open("GET", url, true);
    req.send(null);
}
 
 function AccessoriesProductRequest() {
    if(req.readyState == 4) {    	
        ...
        if (req.status == 200) 
        {		  	
          AccessoriesProductMessages();
        }
    }
}
function AccessoriesProductMessages() 
{	 
    
    var batchs = req.responseXML.getElementsByTagName("AccessoriesProducts")[0];   
    var str=""; 
    ...
    {	     
		...
		str=str+"<td  class='boldEleven'><input name='id' id='id' type='checkbox' value='"+Productid+"'/>&nbsp;"+(loop+1);
		str=str+"<td  class='boldEleven'>&nbsp;"+Product;
		str=str+"<td  class='boldEleven'>&nbsp;"+Group;
		str=str+"<td  class='boldEleven'>&nbsp;"+Desc;
