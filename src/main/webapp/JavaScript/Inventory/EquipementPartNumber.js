...
function LoadEquipementPartNumber(startLetter)
{
	try
	{
	 	var url = "../inventory?actionS=LoadEquipementPartNumber&startLetter="+escape(startLetter);
	 	initRequest(url);
		req.onreadystatechange=LoadEquipementPartNumberRequest;
 ...
 function LoadEquipementPartNumberRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
	    {    	
	    	...
	    	if (req.status == 200) 
	        {		  	
	    		LoadEquipementPartNumberMessages();
	        }
	    }
	}
	catch(err)
...
function LoadEquipementPartNumberMessages() 
{	 
 try
 {
