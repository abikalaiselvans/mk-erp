...
function LoadPaymentCondition(startLetter)
{
	try
	{
	 	var url = "../inventory?actionS=LoadPaymentCondition&startLetter="+escape(startLetter);
		...
		initRequest(url);
		req.onreadystatechange=LoadPaymentConditionRequest;
 ...
 function LoadPaymentConditionRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
...
function LoadPaymentConditionMessages() 
{	 
 try
 {
	 ...
	 str=str+"<td  class='boldEleven'><b>Payment Condition Name </b></td>";
