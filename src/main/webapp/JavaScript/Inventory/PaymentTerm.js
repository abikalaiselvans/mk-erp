...
function LoadPaymentTermView(startLetter)
{
	try
	{
	 	var url = "../inventory?actionS=LoadPaymentTermView&startLetter="+escape(startLetter);
		...
		initRequest(url);
		req.onreadystatechange=LoadPaymentTermViewRequest;
 ...
 function LoadPaymentTermViewRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
...
function LoadPaymentTermViewMessages() 
{	 
 try
 {
	 ...
	 str=str+"<td  class='boldEleven'><center><b>TERMS</b></center></td>";
