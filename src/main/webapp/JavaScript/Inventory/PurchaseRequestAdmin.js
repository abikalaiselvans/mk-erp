...
function loadPurchaseRequestAdmin(startLetter) 
{
	...
	var url = "../inventory?actionS=loadPurchaseRequestAdmin&Branch="+escape(Branch)+"&id="+escape(startLetter)+"&division="+division+"&month="+month+"&customer="+customer+"&year="+year+"&status="+status+"&day="+day;
	initRequest(url);
   	req.onreadystatechange =loadPurchaseRequestAdminRequest;
   	req.open("GET", url, true);
    req.send(null);
}

function loadPurchaseRequestAdminRequest() 
{	
	if(req.readyState == 4) 
    {    	
       ...
       if (req.status == 200)
        {
    	   loadPurchaseRequestAdminMessages();
        }
    }
}


function loadPurchaseRequestAdminMessages() 
{
	var str=""; 
	...
	for(loop=0; loop<batchs.childNodes.length; loop++) 
    {
		...
		if(Bh == "N")
    		...
    		str=str+"<td class='boldEleven'><a href='PurchaseRequestAdminAdd.jsp?Requestno="+Requestno+"'><img src='../images/inventory/Admin_Appr.png' ></a></td>";
		else if(Stock == "N")
