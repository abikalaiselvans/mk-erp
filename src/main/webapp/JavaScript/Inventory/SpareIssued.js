...
function LoadSpareIssued(startLetter)
{
	try
	{
		...
		var url = "../inventory?actionS=LoadSpareIssued&startLetter="+escape(startLetter)+"&branch="+branch+"&calltype="+calltype+"&month="+month+"&year="+year+"&day="+day;
	 	 
	 	initRequest(url);
		req.onreadystatechange=LoadSpareIssuedRequest;
 ...
 function LoadSpareIssuedRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
...
function LoadSpareIssuedMessages() 
{	 
 try
 {
	 ...
	 str=str+"<td><div align='center' class='boldEleven'><b>Status</b></td>";
