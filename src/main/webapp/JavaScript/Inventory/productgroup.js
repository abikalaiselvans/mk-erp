...
function LoadProductGroup(startLetter)
{
	try
	{
	 	var url = "../inventory?actionS=LoadProductGroup&startLetter="+escape(startLetter);
		...
		initRequest(url);
		req.onreadystatechange=LoadProductGroupRequest;
 ...
 function LoadProductGroupRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
...
function LoadProductGroupMessages() 
{	 
 try
 {
