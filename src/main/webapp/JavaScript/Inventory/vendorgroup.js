...
function LoadVendorGroup(startLetter)
{
	try
	{
	 	var url = "../inventory?actionS=LoadVendorGroup&startLetter="+escape(startLetter);
		...
		initRequest(url);
		req.onreadystatechange=LoadVendorGroupRequest;
 ...
 function LoadVendorGroupRequest() 
 {
 	try
 	{
	    if(req.readyState == 4) 
...
function LoadVendorGroupMessages() 
{	 
 try
 {
