...
function LoadPettyCashExpense(startLetter)
{
	var branch = document.getElementById('branch').value;
	var day = document.getElementById('day').value;
	var month = document.getElementById('month').value;
	var year = document.getElementById('year').value;
	var url = "../management?actionS=LoadPettyCashExpense&startLetter="+escape(startLetter)+"&branch="+branch+"&day="+day+"&month="+month+"&year="+year;;  
	initRequest(url);
   	req.onreadystatechange = LoadPettyCashExpenseRequest;
    req.open("GET", url, true);
    req.send(null);    
}	
 

function LoadPettyCashExpenseRequest()
 {
    ...
    if(req.readyState == 4) 
    {
        ...
        if (req.status == 200) 
        {
        	LoadPettyCashExpenseMessage();
        }
    }
}

function LoadPettyCashExpenseMessage() 
{	
    ...
    for(loop=0; loop<batchs.childNodes.length; loop++)
     {
        ...
        str=str+"<td class='boldEleven' align='center'>"+Entry+"</td>";
        sum = sum +parseFloat(Amount);
		str=str+"</tr>";
     }
     str=str+"<tr class='MRow1'>";
     str=str+"<td class='boldEleven' colspan='6' align='right'><font class='bolddeepred'>Total</font></td>";
