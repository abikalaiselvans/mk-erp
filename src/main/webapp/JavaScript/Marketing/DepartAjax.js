...
function loadDepart(startLetter) 
{
	var url = "../marketing?actionS=loadDepart&startLetter="+escape(startLetter);
	initRequest(url);
...
{	 
	var batchs = req.responseXML.getElementsByTagName("RowsDepart")[0];   
    var str=""; 
    str=str+"<table width='100%'  class='boldEleven'  id='myTable'  cellpadding=2 cellspacing=1 bgcolor='#9900CC' >";
    for(loop = 0; loop < batchs.childNodes.length; loop++) 
    {
	    var batch = batchs.childNodes[loop];
	    var Id = batch.getElementsByTagName("DepartId")[0].childNodes[0].nodeValue;
	    var Name = batch.getElementsByTagName("DepartName")[0].childNodes[0].nodeValue;
	    if(loop%2==1)
    ...
    if(batchs.childNodes.length <1)
  	...
  	str=str+"</table>";
	var tb=document.getElementById('DepartmentTable');
  	tb.innerHTML=str;
