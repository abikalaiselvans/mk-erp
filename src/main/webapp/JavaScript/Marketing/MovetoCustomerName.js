...
function LoadCustomerGroup(startLetter)
{
	try
	{
		...
		var url = "../marketing?actionS=LoadMovetoCustomerName&startLetter="+escape(startLetter)+"&status="+status+"&division="+division;
...
function LoadCustomerGroups(ctr)
{
	try
	{
	 	var status = document.getElementById('status').value;
	 	var division = document.getElementById('division').value;
	 	var url = "../marketing?actionS=LoadMovetoCustomerName&startLetter="+escape(ctr.value)+"&status="+status+"&division="+division;
	 	var tob=document.getElementById('divscroll');
		tob.innerHTML="<br><br><center> <img src='../images/loading_gif.gif' width='508' height='381'></center>" ;
		initRequest(url);
		req.onreadystatechange=LoadCustomerGroupRequest;
 ...
 try
 {
	 ...
	 {	     
		...
		if(MovetoInventory == "N")
			str=str+"<td  class='boldEleven' align='center'><a href='../SmartLoginAuth?filename=CustomerName&actionS=MKTMovetoCustomerName&customerrowid="+Rowid+"'><font class='boldgreen'>Move to inventory</font></a></td>";
		else
