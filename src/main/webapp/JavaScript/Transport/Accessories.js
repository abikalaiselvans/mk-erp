...
{
 	var url = "../Transport?actionS=LoadAccessories&startLetter="+escape(startLetter);
 	initRequest(url);
	req.onreadystatechange=LoadAccessoriesRequest;
    req.open("GET", url, true);
    req.send(null);
}
 

function LoadAccessoriesRequest() 
{
    if(req.readyState == 4) 
    {    	
        ...
        if (req.status == 200) 
        {		  	
          LoadAccessoriesMessages();
        }
    }
}


function LoadAccessoriesMessages() 
{	 
