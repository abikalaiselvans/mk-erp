...
function LoadVechicleInfo(startLetter)
{
 	...
 	var url = "../Transport?actionS=LoadVechicleInfo&startLetter="+escape(startLetter)+"&typeid="+typeid+"&makeid="+makeid+"&modelid="+modelid+"&varriant="+varriant
 	initRequest(url);
	req.onreadystatechange=LoadVechicleInfoRequest;
    req.open("GET", url, true);
    req.send(null);
}
 

function LoadVechicleInfoRequest() 
{
    if(req.readyState == 4) 
    {    	
        ...
        if (req.status == 200) 
        {		  	
          LoadVechicleInfoMessages();
        }
    }
}


function LoadVechicleInfoMessages() 
{	 
