...
function LoadSearchStaff( )
{
	try
 	...
 	if(f)
 	{
 		...
 		var url = "../SmartCommonAjax?stype=1&actionS=loadStaffPFESI&search="+escape(search) ;
 		initRequest(url);
...
function loadStaffPFESI(ctr)
{
 	...
 	searchname =  search.toUpperCase(); 	
	var url = "../SmartCommonAjax?stype=2&actionS=loadStaffPFESI&search="+escape(search) ;
	initRequest(url);
	req.onreadystatechange = loadStaffPFESIRequest;
	req.open("GET", url, true);
	req.send(null);
...
{	
    ...
    str=str+"</tr> ";
     ...
     for(loop=0; loop<batchs.childNodes.length; loop++)
     {
        ...
        str=str+"<font "+t+"><a href='Staff_PFandESI_ChangesAction.jsp?staffid="+StaffId+"'>"+s+"</a></font>  </td>";
		...
		//PF,PFtype,PFnumber,Nominee,ESI,ESIType,ESINumber,Resigned
