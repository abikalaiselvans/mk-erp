...
function loadEmd(startLetter) 
{
	try 
	{
		...
		tb.innerHTML = "";
		var url = "../StaffCommon?actionS=INVloadStaffEMD&startLetter="
...
function loadEmdViewMessages() 
{
	try
	{
		...
		for (loop = 0; loop < batchs.childNodes.length; loop++) 
		{
			...
			if (Approval == "N")
				str = str + "<td  class='boldEleven' ><input id='Rowid' name='Rowid' type='checkbox' value='" + Emdid + "' />" + (loop + 1) + ".</td>";
			else
				str = str + "<td  class='boldEleven' width=40>" + (loop + 1)+ ".</td>";
			
			str = str + "<td  class='boldEleven' width=60>" + Emdnumber+ "</td>";
