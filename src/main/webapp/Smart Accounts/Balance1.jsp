  ...
  <tr>
	...
	if(creditdata.length>0)
	{
		...
		for(int u=0;u<creditdata.length;u++)
		{
			...
			sql ="SELECT INT_TYPESUBSIDIARYID FROM accc_m_typesubsidiarygroup WHERE INT_TYPEGROUPID="+creditdata[u][0];
			//out.println(sql);
			...
			if(data.length>0)
			{
				...
				sql = "SELECT SUM(a.DOU_AMOUNT) FROM accc_t_dayentry a WHERE INT_TYPESUBSIDIARYID in("+s+")";
				//out.println(sql);
			...
			out.println("<td class='boldEleven'  align='right'>"+s1);	
	...
	if(debitdata.length>0)
	{
		...
		for(int u=0;u<debitdata.length;u++)
		{
			...
			sql ="SELECT INT_TYPESUBSIDIARYID FROM accc_m_typesubsidiarygroup WHERE INT_TYPEGROUPID="+debitdata[u][0];
			//out.println(sql);
			String data[][] =  com.my.org.erp.common.CommonFunctions.QueryExecute(sql);
			sum=0;
			 s2="0.0";
			...
			out.println("<td class='boldEleven' align='right'>"+s2);	
			out.println("<td class='boldEleven'>&nbsp;");	
			sno = sno+1;
