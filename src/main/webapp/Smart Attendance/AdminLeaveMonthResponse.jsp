...
<%
	...
	reportheader=reportheader+" <b> Leave Report From - </b> "+fdate+" <b>To -</b> "+tdate;
	String sorting = request.getParameter("sorting");
	String ss = request.getParameter("ss");
	  
	String sql="SELECT  a.CHR_EMPID ,b.CHR_STAFFNAME,DATE_FORMAT(a.DT_LDATE,'%d-%m-%Y'),  a.CHR_DAYTYPE, c.CHR_LEAVENAME,a.CHR_LEAVESTATUS ";
	...
	sql=sql+"  AND a.DT_LDATE>='"+fdate+"'  AND a.DT_LDATE<='"+tdate+"'";
	sql = sql+" and b.CHR_TYPE<>'T' AND C.CHR_SYMBOL=a.CHR_LEAVETYPE  ORDER BY DT_LDATE,"+ sorting +" "+ss;
...
%>
 ...
 <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="LEAVE TYPE" sortable="true"><%=temp.elementAt(5)%></display:column>
					<display:column title="STATUS" sortable="true"><%=temp.elementAt(6)%></display:column>
					
					 
					 
					<display:setProperty name="export.csv.filename" value="AdminODMonth.csv"/>
					<display:setProperty name="export.excel.filename" value="AdminODMonth.xls"/>
					<display:setProperty name="export.xml.filename" value="AdminODMonth.xml"/>
					<display:setProperty name="export.pdf.filename" value="AdminODMonth.pdf"/>
					<display:setProperty name="export.pdf" value="true" />
					 
</display:table>     

<br />	<center>	<a href='AdminLeaveMonth.jsp'> CLOSE</a></center>
