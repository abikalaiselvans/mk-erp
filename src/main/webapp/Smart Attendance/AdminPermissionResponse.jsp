...
<%
	...
	reportheader=reportheader+" <b> Permission Report From - </b> "+fdate+" <b>To -</b> "+tdate;
	String sorting = request.getParameter("sorting");
	String ss = request.getParameter("ss");
	 
	String sql="SELECT  a.CHR_EMPID ,b.CHR_STAFFNAME,DATE_FORMAT(a.DT_PDATE,'%d-%m-%Y'), ";
	sql=sql+" a.DT_TIME_FROM, a.DT_TIME_TO, a.CHR_PTYPE,a.CHR_REASON ";
	fdate=DateUtil.FormateDateSQL(fdate);
	tdate=DateUtil.FormateDateSQL(tdate);
	 sql = sql+" From att_t_permission a, com_m_staff b ";
	...
	if(!"0".equals(dept))
	 ...
	 sql=sql+"    AND a.DT_PDATE>='"+fdate+"'  AND a.DT_PDATE<='"+tdate+"'";
 ...
 String data[][] =com.my.org.erp.common.CommonFunctions.QueryExecute(sql);
     ...
     for(int u=0; u<data.length;u++)
	 {
			...
			if("F".equals(data[u][5]))
				child.addElement("Forenoon");
			else if("A".equals(data[u][5]))
				child.addElement("Afternoon");
			else 
				child.addElement("Full Day");	
...
%>
 ...
 <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="TYPE" sortable="true"><%=temp.elementAt(6)%></display:column>
					<display:column title="REASON" sortable="true"><%=temp.elementAt(7)%></display:column>
					 
					 
					
					<display:setProperty name="export.csv.filename" value="AdminPermissionMonth.csv"/>
					<display:setProperty name="export.excel.filename" value="AdminPermissionMonth.xls"/>
					<display:setProperty name="export.xml.filename" value="AdminPermissionMonth.xml"/>
					<display:setProperty name="export.pdf.filename" value="AdminPermissionMonth.pdf"/>
					<display:setProperty name="export.pdf" value="true" />
					 
</display:table>     

<br />	<center>	<a href='AdminPermissionMonth.jsp'> CLOSE</a></center>
