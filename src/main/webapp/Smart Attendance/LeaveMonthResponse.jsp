...
<%
	...
	reportheader=reportheader+" <b> Leave Report For </b> "+umonth+"-"+year;
	String sorting = request.getParameter("sorting");
	String ss = request.getParameter("ss");
    String sql="SELECT  a.CHR_EMPID ,b.CHR_STAFFNAME,a.DOU_LEAVE  ";
	sql = sql+" From att_t_register a, com_m_staff b ";
	sql = sql+" WHERE a.INT_YEAR="+year+" AND a.CHR_EMPID = b.CHR_EMPID AND a.DOU_LEAVE>0  ";
...
%>
	 <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="LEAVE" sortable="true"><%=temp.elementAt(3)%></display:column>
					 
					
		 
					 
					 
					<display:setProperty name="export.csv.filename" value="LeaveMonth.csv"/>
					<display:setProperty name="export.excel.filename" value="LeaveMonth.xls"/>
					<display:setProperty name="export.xml.filename" value="LeaveMonth.xml"/>
					<display:setProperty name="export.pdf.filename" value="LeaveMonth.pdf"/>
