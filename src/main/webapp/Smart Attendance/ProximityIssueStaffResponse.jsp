...
<%
	...
	sql=sql+"D.CHR_DESIGNAME,DATE_FORMAT(A.DT_DOJCOLLEGE,'%d-%m-%Y'),E.CHR_CARDNO  "; 
	sql = sql+" FROM com_m_staff A, com_m_office  B,COM_M_DEPART C,COM_M_DESIG D,att_m_proximitycardno E ";
	sql=sql+" WHERE A.CHR_EMPID in (select CHR_EMPID from att_m_proximitycardno) ";
	...
	sql=sql+" GROUP BY A.CHR_EMPID ORDER BY "+ sorting +" "+ss;
...
%>
 ...
 <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="CARDNO" sortable="true"><%=temp.elementAt(7)%></display:column>
					
					 
				 
					<display:setProperty name="export.csv.filename" value="ProximityIssueStaffInfo.csv"/>
					<display:setProperty name="export.excel.filename" value="ProximityIssueStaffInfo.xls"/>
					<display:setProperty name="export.xml.filename" value="ProximityIssueStaffInfo.xml"/>
					<display:setProperty name="export.pdf.filename" value="ProximityIssueStaffInfo.pdf"/>
					<display:setProperty name="export.pdf" value="true" />
					 
</display:table>     

<br />	<center>	<a href='ProximityIssueStaffInfo.jsp'> CLOSE</a></center>
