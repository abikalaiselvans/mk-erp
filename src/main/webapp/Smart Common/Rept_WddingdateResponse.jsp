...
<%
 	 ...
 	 String reportheader="STAFF DATE OF WEDDING ";
	...
	sql = sql +" DATE_FORMAT(A.DT_WEDDATE,'%d-%b-%Y')  ,DAY(A.DT_WEDDATE), DAY(now()), A.CHR_MOBILE  FROM  com_m_staff  A,  com_m_depart   B, com_m_desig  C , com_m_office D  "; 
	...
	if(!"0".equals(company))
	 ...
	 if(!"0".equals(month))
	 	sql = sql +" AND MONTH(A.DT_WEDDATE)="+month;
	 sql = sql +" ORDER BY  DAY(A.DT_WEDDATE ) ";
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="DOJ" sortable="true"><%=temp.elementAt(7)%></display:column>
					 <display:column title="DOW" sortable="true"><%=temp.elementAt(8)%></display:column>
					...
					<display:setProperty name="export.excel.filename" value="Rept_Wddingdate.xls"/>
					<display:setProperty name="export.pdf.filename" value="Rept_Wddingdate.pdf"/>
					<display:setProperty name="export.csv.filename" value="Rept_Wddingdate.csv"/>
					<display:setProperty name="export.pdf"  value="true" />
					 
</display:table> 
			
<br />	<center>	<a href='Rept_Wddingdate.jsp'> CLOSE</a></center>
