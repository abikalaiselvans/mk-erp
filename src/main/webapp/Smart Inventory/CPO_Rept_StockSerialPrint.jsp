...
try
{
		...
		if("I".equals(itype))
		{
				sql = " SELECT b.CHR_ITEMID,b.CHR_ITEMNAME ,b.CHR_DES,  COUNT(*) ,SUM(a.DOU_UNITPRICE), c.CHR_GROUPNAME ,FUN_INV_DIVISION(a.INT_DIVIID) ,a.INT_DIVIID ";
				...
				sql = sql+ "  AND a.CHR_TYPE='I'  AND a.CHR_FLAG ='N' AND a.CHR_SERIALNO IS NOT NULL ";
				if(!"0".equals(pgroup))
		...
		if("P".equals(itype))
		{
				sql = " SELECT b.CHR_PRODUCTID,b.CHR_PRODUCTCODE ,b.CHR_PRODUCTDESC,  COUNT(*),SUM(a.DOU_UNITPRICE) ,c.CHR_NAME, FUN_INV_DIVISION(a.INT_DIVIID),a.INT_DIVIID    ";
				...
				sql = sql+ "  AND a.CHR_TYPE='P'  AND a.CHR_FLAG ='N'   AND b.CHR_PTYPE !='S'   AND a.CHR_SERIALNO IS NOT NULL ";
				if(!"0".equals(pgroup))
				...
				if(data.length>0)
				{
					...
					for(int u=0;u<data.length;u++)
					{
						...
						if(count>0)
						{
								...
								sql = sql+ "  AND a.CHR_TYPE='P'   AND a.CHR_FLAG  ='N' AND a.CHR_SERIALNO IS NOT NULL";
								if(!"0".equals(Branch))
									sql = sql+ "  AND  a.INT_BRANCHID ="+Branch;
								sql = sql+ "  AND  a.CHR_ITEMID ='"+data[u][0]+"'";
								sql = sql+ "  AND  a.INT_DIVIID ="+data[u][7];
