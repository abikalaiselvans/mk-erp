...
try
{
		...
		if("C".equals(salestype))
		{
			...
			sql = sql +" a.CHR_DESCRIPTION,a.DOU_BALANCEAMOUNT,a.CHR_STATUS,a.CHR_PAYMENTFLAG,a.CHR_NEXTFLAG,b.DOU_TOTALAMOUNT,FIND_A_CUSTOMER_NAME(b.INT_CUSTOMERID) ,d.CHR_DIVICODE ";
			...
			sql = sql +" AND b.INT_DIVIID= d.INT_DIVIID AND a.CHR_INVOICETYPE='C'	AND e.INT_COMPANYID=f.INT_COMPANYID AND b.INT_BRANCHID =f.INT_BRANCHID 		AND a.CHR_PAYMENTFLAG != 'Y' 	 ";
		...
		else if("D".equals(salestype))
		{
			...
			sql = " SELECT a.INT_PAYMENTCOMMITMENTID, a.CHR_SALESNO, a.CHR_INVOICETYPE, DATE_FORMAT(b.DAT_SALESDATE,'%d-%b-%Y'), ";
			sql = sql +" DATEDIFF(a.DT_COMMITMENTDATE,b.DAT_SALESDATE), DATE_FORMAT(a.DT_COMMITMENTDATE,'%d-%b-%Y'),a.DOU_COMMITMENTAMOUNT,a.DT_RECIEVEDDATE,a.DOU_RECEIVEDAMOUNT, ";
			sql = sql +" a.CHR_DESCRIPTION,a.DOU_BALANCEAMOUNT,a.CHR_STATUS,a.CHR_PAYMENTFLAG,a.CHR_NEXTFLAG,b.DOU_NETAMOUNT,FIND_A_CUSTOMER_NAME(b.INT_CUSTOMERID)  ,d.CHR_DIVICODE ,e.CHR_COMPANYNAME ,f.CHR_BRANCHNAME  ";
			...
			sql = sql +" AND b.INT_DIVIID= d.INT_DIVIID AND a.CHR_INVOICETYPE='D'		AND a.CHR_PAYMENTFLAG != 'Y' 		 ";
			sql = sql+" AND  DT_COMMITMENTDATE  >= '"+com.my.org.erp.ServiceLogin.DateUtil.FormateDateSQL(fromdate)+"'";	
			sql = sql+" AND  DT_COMMITMENTDATE  <= '"+com.my.org.erp.ServiceLogin.DateUtil.FormateDateSQL(todate)+"'";	
			//if(!"0".equals(Branch))
				//sql = sql+" AND b.INT_BRANCHID ="+Branch;
		...
		else if("S".equals(salestype))
		{
			...
			sql = " SELECT a.INT_PAYMENTCOMMITMENTID, a.CHR_SALESNO, a.CHR_INVOICETYPE, DATE_FORMAT(b.DAT_SALESDATE,'%d-%b-%Y'), ";
			sql = sql +" DATEDIFF(a.DT_COMMITMENTDATE,b.DAT_SALESDATE), DATE_FORMAT(a.DT_COMMITMENTDATE,'%d-%b-%Y'),a.DOU_COMMITMENTAMOUNT,a.DT_RECIEVEDDATE,a.DOU_RECEIVEDAMOUNT, ";
			sql = sql +" a.CHR_DESCRIPTION,a.DOU_BALANCEAMOUNT,a.CHR_STATUS,a.CHR_PAYMENTFLAG,a.CHR_NEXTFLAG,b.DOU_NETAMOUNT,FIND_A_CUSTOMER_NAME(b.INT_CUSTOMERID)  ,d.CHR_DIVICODE   ";
			sql = sql +"  ,e.CHR_COMPANYNAME ,f.CHR_BRANCHNAME  FROM inv_t_paymentcommitment a,inv_t_servicebilling b,inv_m_customerinfo c,inv_m_division d  ,com_m_company e, com_m_branch f  WHERE a.CHR_SALESNO=b.CHR_SALESNO AND b.INT_CUSTOMERID =c.INT_CUSTOMERID AND e.INT_COMPANYID=f.INT_COMPANYID AND b.INT_BRANCHID =f.INT_BRANCHID ";
			sql = sql +"  AND b.INT_DIVIID= d.INT_DIVIID AND a.CHR_INVOICETYPE='S'	AND a.CHR_PAYMENTFLAG != 'Y' 	 ";
			sql = sql+" AND  DT_COMMITMENTDATE  >= '"+com.my.org.erp.ServiceLogin.DateUtil.FormateDateSQL(fromdate)+"'";	
			sql = sql+" AND  DT_COMMITMENTDATE  <= '"+com.my.org.erp.ServiceLogin.DateUtil.FormateDateSQL(todate)+"'";	
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="CommitmentDate" sortable="true"><%=temp.elementAt(7)%></display:column>
					<display:column title="Day Difference" sortable="true"><%=temp.elementAt(8)%></display:column>
					<display:column title="Invoice Amount" sortable="true"><%=temp.elementAt(9)%></display:column>
					<display:column title="Commitment Amount" sortable="true"><%=temp.elementAt(10)%></display:column>
					 

					<display:column title="Received Date" sortable="true"><%=temp.elementAt(11)%></display:column>
					<display:column title="Received Amount" sortable="true"><%=temp.elementAt(12)%></display:column>
					<display:column title="Description" sortable="true"><%=temp.elementAt(13)%></display:column>
					
					<display:column title="Balance" sortable="true"><%=temp.elementAt(14)%></display:column>
					<display:column title="Commitment Status" sortable="true"><%=temp.elementAt(15)%></display:column>
					
					<display:column title="Payment Status" sortable="true"><%=temp.elementAt(16)%></display:column>
					<display:column title="Postpone Status" sortable="true"><%=temp.elementAt(17)%></display:column>
