...
<%@ page import="com.my.org.erp.Decorator.SmartInventory.*"%>
...
sql = sql +" a.DAT_SWAPDATE,a.CHR_SERIALNO, ";
...
SwappingInfo sData = new SwappingInfo();
Vector mn = new SwappingInfo().loadData1(sql);
request.setAttribute("table",mn);


/*
 
ArrayList al = new SwappingInfo().loadData(sql);
out.println(al);
request.setAttribute("table",al);

 
<display:table   id="_table" name="table"  export="true" pagesize="25"decorator="com.my.org.erp.Decorator.SmartInventory.SwappinInfoDecorator" >
 
			<display:column property="sno" title="TV Show" />
			<display:column property="branch" title="User Name" />
			<display:column property="swappingserial" title="Email Id" />
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="SWAPPING SERIAL NUMBER"   sortable="true"><%=temp.elementAt(10)%></display:column>
					<display:column media="html" title="URL"><%="<A HREF='SS.HTML' TARGET='_BLANK'>"+temp.elementAt(10)+"</A>"%> 
    </display:column>
