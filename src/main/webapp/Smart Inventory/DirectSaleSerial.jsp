...
{ 
	...
	//out.println("SELECT "+field+" FROM inv_t_directsales WHERE CHR_SALESNO='"+salesno+"'") ;
...
String vendor[][]=CommonFunctions.QueryExecute(sql);
						...
						%>
								...
								<a onClick="javascript:window.print()">sasa</a>
...
h1=" onKeyUp=\"upperMe(this),LoadUniqueCheckSales('serial"+(v+1)+"','Valid"+(v+1)+"' , 'inv_t_vendorgoodsreceived','CHR_SERIALNO','Submit','"+recdData[v][5]+"','"+recdData[v][2]+"')\" ";
		...
		out.print("<td><input name='serial"+row+"'  class='formText135' maxlength='50'   size=20 type='text' value='"+recdData[v][3]+"'   "+h1+"  "+h+">    </td>");
		out.println("<td>&nbsp; <div id='Valid"+row+"'></DIV></TD>");
		out.print("<td class='boldEleven'>&nbsp;<div id='Warranty"+row+"'></DIV>");
		
		out.print("<input name='item"+row+"' type='hidden'  value='"+recdData[v][2]+"'>");
		out.print("<input name='itype"+row+"' type='hidden'  value='"+recdData[v][5]+"'>");
		out.print("<input name='rowid"+row+"' type='hidden' value='"+recdData[v][0]+"'>");
		out.print("<input name='Warrantys"+row+"' id='Warrantys"+row+"'  type='hidden'  value='1'>");
							...
							<table width="147" border="0" align="center" cellpadding="3"
								...
								<tr>
									<td width="56"><input name="Submit" id="Submit"
										type="submit" class="buttonbold" value="Submit"   accesskey="s"   ></td>
