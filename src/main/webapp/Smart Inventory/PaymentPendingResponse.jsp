...
try
{
		...
		double balancetotalamount=0.00;
		double sum=0.00;	
		 //CUSTOMER SALES 
			...
			if(data.length>0)
			{
				 for(int y=0;y<data.length;y++)
				{
					...
					if("P".equals(data[y][4]))
					{
						...
						balancetotalamount = balancetotalamount+  Math.round(Double.parseDouble(data[y][3])-pamount);
					...
					else
					{
						...
						balancetotalamount = balancetotalamount+  Math.round(Double.parseDouble(data[y][3]));
	...
	if(mn.size()>0){
			...
			Double a = balancetotalamount;
			 String result = String.format("%.2f", balancetotalamount);
			//child.addElement(""+UtilityFunctions.ConvertExponentialformtoDecimalnumber(a));
			child.addElement(result);
			child.addElement("");
