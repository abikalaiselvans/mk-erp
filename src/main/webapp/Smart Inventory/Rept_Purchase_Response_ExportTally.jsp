		...
		if(("1".equals(type)) || ("0".equals(type) ))
		{
				...
				if(mdata.length>0)
				{
					...
					for(int j=0;j<mdata.length;j++)
					{
						...
						sql = sql + " CONCAT('CR-',a.INT_VENDORID) CR, CONCAT(b.CHR_VENDORNAME) vendor,   ";//,'-','CR-',a.INT_VENDORID
						sql = sql + " a.CHR_VENDORPO,   ";
						sql = sql + " CONCAT('CORPORATE'),    ";
						sql = sql + " REPLACE(RIGHT(a.CHR_PURCHASEORDERNO,7),'/','-') costcenter,   ";
						sql = sql + " CONCAT(LEFT(a.CHR_PURCHASEORDERNO,2),' Purchase @ ',d.CHR_TAXNAME ) ledger,  ";
						sql = sql + " SUM((INT_QUANTITY*DOU_UNITPRICE)) ledgervalue,   ";
						sql = sql + " CONCAT('DR'),    ";
						sql = sql + " REPLACE(LEFT(a.CHR_DES,200),'\n',''),IF(a.CHR_BASEVALUE='Y','Check','')  ";    
						...
						sql = sql + " SELECT   ";    
						sql = sql + " a.CHR_PURCHASEORDERNO po, DATE_FORMAT(a.DAT_ORDERDATE,'%d-%b-%Y') dt,   ";   
						sql = sql + " CONCAT('CR-',a.INT_VENDORID) CR, CONCAT(b.CHR_VENDORNAME ) vendor,  ";    
						sql = sql + " a.CHR_VENDORPO,   ";
						sql = sql + " CONCAT('CORPORATE'),     ";  
						sql = sql + " REPLACE(RIGHT(a.CHR_PURCHASEORDERNO,7),'/','-') costcenter,  ";
						sql = sql + " CONCAT(LEFT(a.CHR_PURCHASEORDERNO,2),' Purchase Input Tax  @ ',d.CHR_TAXNAME) ledger,  ";   
						sql = sql + " SUM(c.DOU_TAX_AMOUNT) ledgervalue, CONCAT('DR'),   ";  
						sql = sql + " REPLACE(LEFT(a.CHR_DES,200),'\n','') ,IF(a.CHR_BASEVALUE='Y','Check','')    ";   
						sql = sql + " FROM inv_t_directpurchase a, inv_m_vendorsinformation b, inv_t_directpurchaseitem c ,inv_m_tax d  ";   
						sql = sql + " WHERE a.INT_VENDORID = b.INT_VENDORID   ";  
						sql = sql + " AND c.INT_TAXID = d.INT_TAXID AND c.DOU_TAX_PERCENTAGE >0    "; 
						...
						sql = sql + "  REPLACE(RIGHT(a.CHR_PURCHASEORDERNO,7),'/','-') costcenter,    ";
						sql = sql + "  CONCAT(LEFT(a.CHR_PURCHASEORDERNO,2),' Purchase @ ',d.CHR_TAXNAME) ledger,    "; 
						sql = sql + " SUM(c.INT_QUANTITY*c.DOU_UNITPRICE) ledgervalue, CONCAT('DR'),   ";  
						sql = sql + " REPLACE(LEFT(a.CHR_DES,200),'\n','') ,IF(a.CHR_BASEVALUE='Y','Check','')      "; 
						sql = sql + " FROM inv_t_directpurchase a, inv_m_vendorsinformation b, inv_t_directpurchaseitem c , inv_m_tax d     ";
						sql = sql + " WHERE a.INT_VENDORID = b.INT_VENDORID     ";
						sql = sql + " AND a.CHR_PURCHASEORDERNO =c.CHR_PURCHASEORDERNO  AND c.DOU_TAX_PERCENTAGE =0   ";
						...
						sql = sql + "  CONCAT('CORPORATE'),   "; 
						sql = sql + "   REPLACE(RIGHT(a.CHR_PURCHASEORDERNO,7),'/','-') costcenter,   ";
						sql = sql + "   CONCAT(LEFT(a.CHR_PURCHASEORDERNO,2),' Purchase Input @ ',d.CHR_TAXNAME) ledger,    ";
						sql = sql + "  a.DOU_TAXAMOUNT ledgervalue, CONCAT('DR'),   "; 
						sql = sql + "  REPLACE(LEFT(a.CHR_DES,200),'\n','') ,IF(a.CHR_BASEVALUE='Y','Check','')    "; 
						...
						sql = sql + " CONCAT('CORPORATE'),    ";
						sql = sql + " REPLACE(RIGHT(a.CHR_PURCHASEORDERNO,7),'/','-') costcenter,   ";
						sql = sql + " CONCAT(LEFT(a.CHR_PURCHASEORDERNO,2),' Purchase Input @  Frieght' ) ledger,   ";
						sql = sql + " a.DOU_FRIEGHT_CHARGE ledgervalue, CONCAT('DR'),    ";
						sql = sql + " REPLACE(LEFT(a.CHR_DES,200),'\n','') ,IF(a.CHR_BASEVALUE='Y','Check','')  ";   
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="BASE-VALUE"   sortable="true"><%=temp.elementAt(13)%></display:column>
