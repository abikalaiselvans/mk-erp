...
try
{
 		String reportheader="PURCHASE REPORT "; 
		String sql ="";
		...
		{
		 	sql = sql + " (SELECT  a.CHR_VENDORPO InvNumber, DATE_FORMAT(a.DAT_ORDERDATE,'%d-%b-%Y') InvDate, e.CHR_VENDORNAME NameoftheVendor,   e.CHR_GSTNO  VendorGSTNo,  ";
			sql = sql + " j.CHR_MODELCODE Brand ,  f.CHR_GROUPNAME ProductGroup, c.CHR_DES PCODEDESCRIPTION ,    ";
			sql = sql + " FUN_INV_GET_WARRANTY_IN_YEAR(b.INT_WARRANTY) Warranty,  c.CHR_HSNCODE HSNorSAC ,  FUN_INV_GET_PURCHASE_SERIALNUMBER_CONCATS(b.CHR_TYPE,b.CHR_ITEMID, a.CHR_PURCHASEORDERNO) SlNos,  ";
			sql = sql + " b.INT_QUANTITY Qty, b.DOU_UNITPRICE UnitCost, ROUND((b.INT_QUANTITY  * b.DOU_UNITPRICE),2) BeforeTax,  b.DOU_TAX_AMOUNT TaxAmt,  b.DOU_TOTAL Total, a.DOU_AMOUNT NetValue,   ";
			...
			sql = sql + " from inv_t_directpurchase a,inv_t_directpurchaseitem b,inv_m_item c,  inv_m_division d,inv_m_vendorsinformation e,inv_m_itemgroup f, com_m_staff g ,  ";
			sql = sql + " inv_m_vendorsgroup i, inv_m_model j, inv_m_tax k  ";
			sql = sql + " WHERE a.CHR_PURCHASEORDERNO = b.CHR_PURCHASEORDERNO   ";
			sql = sql + " AND  b.CHR_ITEMID =c.CHR_ITEMID   AND b.CHR_TYPE ='I'    ";
			...
			sql = sql + " AND e.INT_GROUPID = i.INT_GROUPID  ";
			...
			sql = sql + "  (SELECT a.CHR_VENDORPO VendorInvoice, DATE_FORMAT(a.DAT_ORDERDATE,'%d-%b-%Y') PODate, e.CHR_VENDORNAME NameoftheVendor,   e.CHR_GSTNO  VendorGSTNo,  ";
			sql = sql + "  j.CHR_MODELCODE Brand ,   b1.CHR_NAME ProductGroup, c.CHR_PRODUCTDESC PCODEDESCRIPTION  ,   ";
			sql = sql + " FUN_INV_GET_WARRANTY_IN_YEAR(b.INT_WARRANTY) Warranty,  c.CHR_HSNCODE HSNorSAC ,  FUN_INV_GET_PURCHASE_SERIALNUMBER_CONCATS( b.CHR_TYPE,b.CHR_ITEMID,a.CHR_PURCHASEORDERNO) SlNos,  ";
			sql = sql + " b.INT_QUANTITY Qty, b.DOU_UNITPRICE UnitCost, ROUND((b.INT_QUANTITY  * b.DOU_UNITPRICE),2) BeforeTax,  b.DOU_TAX_AMOUNT TaxAmt,  b.DOU_TOTAL Total, a.DOU_AMOUNT NetValue,   ";
			sql = sql + "  ROUND(a.DOU_TOTALAMOUNT,2)  FinalValue , k.CHR_TAXNAME GST,g.CHR_STAFFNAME AccountManager  , FUN_GET_BRANCH_NAME(a.INT_BRANCHID)  branchname ";
			sql = sql + " from inv_t_directpurchase a,inv_t_directpurchaseitem b,inv_m_produtlist c,  inv_m_productgroup b1, inv_m_division d,inv_m_vendorsinformation e,inv_m_productgroup f, com_m_staff g ,  ";
			sql = sql + "  inv_m_vendorsgroup i, inv_m_model j, inv_m_tax k  ";
			sql = sql + " WHERE a.CHR_PURCHASEORDERNO = b.CHR_PURCHASEORDERNO  ";  
			sql = sql + " AND b.CHR_ITEMID =c.CHR_PRODUCTID   AND b.CHR_TYPE ='P'    ";
			sql = sql + " AND a.INT_DIVIID= d.INT_DIVIID   ";
			sql = sql + " AND a.INT_VENDORID = e.INT_VENDORID   ";
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
 <display:caption><%=reportheader.toUpperCase()%></display:caption>
					...
					<display:column title="NAMEOFTHEVENDOR" sortable="true"><%=temp.elementAt(2)%></display:column>
					<display:column title="VENDORGSTNO"   sortable="true"><%=temp.elementAt(3)%></display:column>
					...
					<display:column title="FINALVALUE"  style="text-align:right" sortable="true"><%=temp.elementAt(16)%></display:column>
					<display:column title="GST"   style="text-align:right" ><%=temp.elementAt(17)%></display:column>  
					<display:column title="ACCOUNTMANAGER"   sortable="true"><%=temp.elementAt(18)%></display:column>  
					<display:column title="BRANCH"   sortable="true"><%=temp.elementAt(19)%></display:column>  
		 	
					<display:setProperty name="export.excel.filename" value="Rept_Purchasedetails_Template.xls"/>
					<display:setProperty name="export.pdf.filename" value="Rept_Purchasedetails_Template.pdf"/>
					<display:setProperty name="export.csv.filename" value="Rept_Purchasedetails_Template.csv"/>
					<display:setProperty name="export.pdf"  value="true" />
					 
					 
</display:table> 
    
<br />	<center>	<a href='Rept_Purchasedetails.jsp'> CLOSE</a>
</center>
 <%
}
catch(Exception e)
