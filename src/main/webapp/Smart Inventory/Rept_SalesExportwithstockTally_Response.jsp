...
try
{
		...
		String filename = CommonFunctions.QueryExecute(" SELECT CONCAT('Sales_ExportWithStock_Template_',DATE_FORMAT(now(),'%Y%m%d%h%m%s') )")[0][0];
		...
		{
				sql = "";
				/*sql = sql + " SELECT   a.CHR_SALESNO DOCNO, DATE_FORMAT(a.DAT_SALESDATE,'%d-%b-%Y') DOCDT, a.CHR_SALESNO INVNO, DATE_FORMAT(a.DAT_SALESDATE,'%d-%b-%Y') INVDT, CONCAT('GST Sales') VOCUHERTYPE,   ";
				sql = sql + " CONCAT('DR-',b.INT_CUSTOMERGROUPID) LEDGERCODE, CONCAT(FIND_A_CUSTOMER_NAME(a.INT_CUSTOMERID) ) PARTYLEDGER,   b.CHR_GSTNO GSTINNO  , ";
				sql = sql + " b.CHR_ADDRESS1 ADDRESS1, b.CHR_ADDRESS2 ADDRESS2,  b.CHR_ADDRESS3 ADDRESS3,  g. CHR_CITYNAME CITY , f.CHR_DISTRICT DISTRICT,  e.CHR_STATENAME STATENAME, e.CHR_TINCODE STATECODE,  ";
				sql = sql + " b.INT_PINCODE PINCODE, ";
				sql = sql + " a.DOU_TOTALAMOUNT INVOICEVALUE, ";
				...
				sql = sql + " CONCAT('GST Sales A/c')	SALESLEDGERNAME, ";
				sql = sql + " IF(a.CHR_GST_TYPE='S','STATE','CENTRAL') GSTTYPE, ";
				sql = sql + " CONCAT('') IGST, FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'igst') IGSTAMOUNT, ";
				sql = sql + " CONCAT('') CGST, FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'cgst') CGSTAMOUNT, ";
				sql = sql + " CONCAT('') SGST, FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'sgst') SGSTAMOUNT, ";
				sql = sql + " a.DOU_TCS_PERCENTAGE TCS, a.DOU_TCS_AMOUNT TCSAMOUNT, ";
				sql = sql + " a.INT_TAXID TDS, a.DOU_TAXAMOUNT TDSAMOUNT, a.DOU_FRIEGHT_CHARGE  FREIGHTCHARGE, ";
				...
				sql = sql + " CONCAT('CORPORATE') COSTCAT,  REPLACE(RIGHT(a.CHR_SALESNO,7),'/','-') COSTCENTER, REPLACE(LEFT(a.CHR_DES,200),'\n','') NARRATION, ";
				...
				sql = sql + " FUN_INV_GET_SALES_SERIALNUMBER_CONCAT(a.CHR_SALESNO) SRNO_DETAILS, c.INT_WARRANTY WARRANTY_DETAILS, ";
				sql = sql + " a.CHR_OTHERREF PO_REFERENCE, FIND_A_PAYMENT_TERM(a.INT_PAYMENTTERMID) PAYMENT_TERMS, a.CHR_DESPATCHTHRU DESPATCHED_THRU, a.CHR_DESTINATION DESTINATION, ";
				sql = sql + " a.CHR_CONTACTDETAILS CONTACT_PERSON, a. CHR_CONTACTNO CONTACT_NO, ";
				sql = sql + " CONCAT(FIND_A_CUSTOMER_NAME(a.INT_CUSTOMERID) )CONSIGNEE_NAME, b.CHR_ADDRESS1 CONSIGNEE_ADDRESS1, b.CHR_ADDRESS2 CONSIGNEE_ADDRESS2,  "; 
				sql = sql + " b.CHR_ADDRESS3 CONSIGNEE_ADDRESS3,  g. CHR_CITYNAME CONSIGNEE_CITY , f.CHR_DISTRICT CONSIGNEE_DISTRICT,  e.CHR_STATENAME CONSIGNEE_STATENAME,  ";
				sql = sql + " b.INT_PINCODE CONSIGNEE_PINCODE  ";*/
				...
				sql = sql + " c.DOU_UNITDISCOUNT DISCOUNT, 	((c.INT_QUANTITY*c.DOU_UNITPRICE) - c.DOU_UNITDISCOUNT) AMOUNT,  ";
				sql = sql + " CONCAT('GST Sales A/c')	SALESLEDGERNAME,  ";
				sql = sql + " IF(a.CHR_GST_TYPE !='S', ROUND(d.CHR_PERCENTAGE,2) , '') IGST, IF(a.CHR_GST_TYPE !='S', FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'igst'),'') IGSTAMOUNT,  ";
				sql = sql + " IF(a.CHR_GST_TYPE ='S', ROUND((d.CHR_PERCENTAGE * DOU_CGST/100),2)  , '') CGST, IF(a.CHR_GST_TYPE ='S',  FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'cgst'),'') CGSTAMOUNT,  ";
				sql = sql + " IF(a.CHR_GST_TYPE ='S', ROUND((d.CHR_PERCENTAGE * DOU_SGST/100),2)  , '')  SGST, IF(a.CHR_GST_TYPE ='S', FUN_INV_DIRECT_SALE_TAX_AMOUNT(a.CHR_SALESNO , 'sgst'), '' ) SGSTAMOUNT,  ";
				sql = sql + " IF( a.DOU_TCS_AMOUNT >0 ,a.DOU_TCS_PERCENTAGE , '') TCS, a.DOU_TCS_AMOUNT TCSAMOUNT,  ";
				...
				sql = sql + " SUBSTRING(c.CHR_DESC, 1, 50) PROD_DEC1 , SUBSTRING(c.CHR_DESC, 51, 50) PROD_DEC2 , SUBSTRING(c.CHR_DESC, 101, 50) PROD_DEC3 , SUBSTRING(c.CHR_DESC, 151, 50) PROD_DEC4 ,   ";
				sql = sql + " SUBSTRING(c.CHR_DESC, 200, 50) PROD_DEC5 , SUBSTRING(c.CHR_DESC, 201, 50) PROD_DEC6 , SUBSTRING(c.CHR_DESC, 251, 50) PROD_DEC7 ,   ";
				sql = sql + " FUN_INV_GET_SALES_SERIALNUMBER_CONCATS(c.CHR_TYPE,c.CHR_ITEMID, a.CHR_SALESNO) SRNO_DETAILS,   ";
				...
				sql = sql + " IF(a.CHR_SHIPPING ='D', 'Different', 'Same') CONSIGNEEADD  ";
				 
				sql = sql + " FROM  inv_t_directsales a, inv_m_customerinfo b, inv_t_swapsalesitem c  ,inv_m_tax d , com_m_state  e , com_m_district f, com_m_city g    "; 
				...
				//sql = sql + "  AND c.DOU_TAX_PERCENTAGE >0   ";    
				...
				String data[][] = CommonFunctions.emptyQueryExecute(sql);  //QueryExecute
				if(data.length>0)
				{
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:setProperty name="export.excel.filename" value="Sales_ExportWithStock_Template.xls"/>
					<display:setProperty name="export.pdf.filename" value="Sales_ExportWithStock_Template.pdf"/>
					<display:setProperty name="export.csv.filename" value="Sales_ExportWithStock_Template.csv"/>
...
<br />	<center>	<a href='Rept_Sales_ExportwithstockTally.jsp'> CLOSE</a></center>
 <%
}
catch(Exception e)
{
