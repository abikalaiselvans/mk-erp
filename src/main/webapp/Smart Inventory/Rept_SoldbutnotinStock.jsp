...
<%
	try
	{
		...
		String reportheader=" SOLD SERIAL NUMBER BUT NOT IN STOCK" ; 
		...
		sql = sql +" productdescription, IF(d.CHR_CANCEL='N',d.CHR_SALESNO,CONCAT(d.CHR_SALESNO,'***')),";	
		sql = sql +" FUN_INV_DIVISION(d.INT_DIVIID) division,DATE_FORMAT(d.DAT_SALESDATE,'%d-%b-%Y') Sdate, ";	
		...
		sql = sql +" FROM";	
		sql = sql +" inv_t_swapsalesserialno a  , inv_t_directsales d";	
		sql = sql +" WHERE a.CHR_SERIALNO is NOT NULL AND a.CHR_SALESNO = d.CHR_SALESNO";	
		sql = sql +" AND d.INT_SALESSTATUS =1  AND a.CHR_GRTFLAG='N' ";	 //AND d.CHR_CANCEL ='N' 
		sql = sql +" AND NOT EXISTS(SELECT b.CHR_SERIALNO FROM inv_t_vendorgoodsreceived b WHERE a.CHR_SERIALNO=b.CHR_SERIALNO )";	
		 //out.println(sql);
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:setProperty name="export.excel.filename" value="Rept_SerialnumberSoldButNotStock.xls"/>
					<display:setProperty name="export.pdf.filename" value="Rept_SerialnumberSoldButNotStock.pdf"/>
					<display:setProperty name="export.csv.filename" value="Rept_SerialnumberSoldButNotStock.csv"/>
