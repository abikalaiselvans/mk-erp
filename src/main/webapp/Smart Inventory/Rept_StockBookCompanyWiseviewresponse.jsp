...
try
	  {
		...
		String cdata[][] =com.my.org.erp.common.CommonFunctions.QueryExecute(" SELECT INT_COMPANYID,CHR_COMPANYNAME FROM  com_m_company   WHERE  INT_COMPANYID ="+Company );
		...
		if("1".equals(itype))
			...
			ii="PRODUCT";	
		 String reportheader="STOCK BOOK IN & OUT ("+ii+") FOR "+month.toUpperCase()+" - "+year+"  ( " +cdata[0][1]+" )" ;
		 
    	 Vector mn = new Vector();
		 Vector child= null;
		  if("1".equals(itype)) 
		 {
				sql = " select  c.CHR_COMPANYNAME,a.CHR_PRODUCTCODE,d.CHR_ITEMNAME,a.CHR_DESC,a.CHR_MONTH,a.INT_YEAR,   ";
				sql = sql +" SUM(a.INT_OPEN),SUM(a.DOU_OPENVALUE),SUM(a.INT_IN),SUM(a.DOU_INVALUE),SUM(a.INT_OUT),SUM(a.DOU_OUTVALUE),SUM(a.INT_BALANCE),SUM(a.DOU_PRICE)   ";
				sql = sql +" from inv_t_stockbookinandout a,com_m_branch b,com_m_company c,inv_m_item d   ";
				sql = sql +" WHERE a.INT_BRANCHID=b.INT_BRANCHID   ";
				sql = sql +" AND b.INT_COMPANYID=c.INT_COMPANYID   ";
				sql = sql +" AND a.CHR_PRODUCTCODE =d.CHR_ITEMID   ";
				sql = sql +" AND a.CHR_TYPE='I'   ";
				sql = sql +"  AND c.INT_COMPANYID= "+Company;
		 ...
		 if("2".equals(itype)) 
		 {
				 	 ...
				 	 sql =  "  select  c.CHR_COMPANYNAME,a.CHR_PRODUCTCODE,d.CHR_PRODUCTCODE,a.CHR_DESC,a.CHR_MONTH,a.INT_YEAR,   ";
					 sql = sql +"  SUM(a.INT_OPEN),SUM(a.DOU_OPENVALUE),SUM(a.INT_IN),SUM(a.DOU_INVALUE),SUM(a.INT_OUT),SUM(a.DOU_OUTVALUE),SUM(a.INT_BALANCE),SUM(a.DOU_PRICE)   ";
					 sql = sql +"  from inv_t_stockbookinandout a,com_m_branch b,com_m_company c,inv_m_produtlist d   ";
					 sql = sql +"  WHERE a.INT_BRANCHID=b.INT_BRANCHID   ";
					 sql = sql +"  AND b.INT_COMPANYID=c.INT_COMPANYID   ";
 ...
 %>
   ...
   <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:setProperty name="export.csv.filename" value="StockBookCompanyWiseview.csv"/>
					<display:setProperty name="export.excel.filename" value="StockBookCompanyWiseview.xls"/>
					<display:setProperty name="export.xml.filename" value="StockBookCompanyWiseview.xml"/>
					<display:setProperty name="export.pdf.filename" value="StockBookCompanyWiseview.pdf"/>
...
<br />	<br /><br /><center>	<a href='javascript:window.close()'> CLOSE</a>
 ...
 <%
		...
		out.println("<td colspan= '15' class='boldEleven' align=center><b>STOCK BOOK VIEW FOR "+month.toUpperCase()+" - "+year+"  ( " +cdata[0][1]+" )</b></td>");
