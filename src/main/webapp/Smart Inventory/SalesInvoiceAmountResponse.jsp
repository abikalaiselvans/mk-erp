...
try
{
		...
		if("Direct".equals(payType) || "ALL".equals(payType))
		{
			...
			sql = " SELECT d.CHR_COMPANYNAME, c.CHR_BRANCHNAME, b.CHR_DIVICODE,COUNT(*),  ";
			sql = sql +" SUM(a.DOU_TAXAMOUNT),SUM(a.DOU_TOTALAMOUNT),  ";
			...
			sql = sql +" AND a.DAT_SALESDATE >='"+year+"-04-01'	  ";
			sql = sql +" AND a.DAT_SALESDATE <='"+(Integer.parseInt(year)+1)+"-03-31'	  ";
			if(!"0".equals(Branch))
	...
	if("ServiceBilling".equals(payType) || "ALL".equals(payType))
		{
			sql = " SELECT d.CHR_COMPANYNAME, c.CHR_BRANCHNAME, b.CHR_DIVICODE,COUNT(*),  ";
			sql = sql +" SUM(a.CHR_TAXAMOUNT),SUM(a.DOU_NETAMOUNT),  ";
			sql = sql +" MONTHNAME(a.DAT_SALESDATE),YEAR(a.DAT_SALESDATE)   ";   
	...
	if("DirectBilling".equals(payType) || "ALL".equals(payType))
		{
			sql = " SELECT d.CHR_COMPANYNAME, c.CHR_BRANCHNAME, b.CHR_DIVICODE,COUNT(*),";
			sql = sql +" SUM(a.DOU_AMOUNT),SUM(a.DOU_NETAMOUNT), MONTHNAME(a.DAT_SALESDATE),YEAR(a.DAT_SALESDATE)   ";
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="NO OF INVOICE " sortable="true"><%=temp.elementAt(5)%></display:column>
					...
					<display:setProperty name="export.excel.filename" value="SalesInvoiceAmount.xls"/>
					<display:setProperty name="export.pdf.filename" value="SalesInvoiceAmount.pdf"/>
					<display:setProperty name="export.csv.filename" value="SalesInvoiceAmount.csv"/>
					<display:setProperty name="export.pdf"  value="true" />
					 
</display:table>     
<br />	<center>	<a href='SalesInvoiceAmount.jsp'> CLOSE</a></center><br /><br />
