		...
		{		
			...
			if(its[1] != "CASH" || its[1] != "CASH")
			{
				...
				checkNull('Challanno','Enter '+its[1]+' Number'))
				{
					...
					{
						const response = confirm("Are you sure you want to do continue same page?");
						if (response) {
						 document.getElementById('pagerepeat').value="Y";
							console.log("Ok was pressed");
						} else {
						document.getElementById('pagerepeat').value="N";
							  ...
							  <td height="17" class="boldEleven">
							    ...
							    <table width="100%" border="0" cellspacing="0" cellpadding="0">
							      <tr>
									...
									{
										String repeatsalesnumber[] = request.getParameter("repeatsalesnumber").split(","); 
										if(repeatsalesnumber.length>0) {
											for(int u=0;u<repeatsalesnumber.length; u++)
											{
												ids =ids + "'"+repeatsalesnumber[u]+"' ,";
												salesnumberids = salesnumberids + repeatsalesnumber[u]+",";
											}	
										}
									}
									else
									{
										...
										if(salesnumber.length>0) {
											for(int u=0;u<salesnumber.length; u++)
											{
												...
												salesnumberids = salesnumberids + salesnumber[u]+",";
											}	
										}
									}
...
<%
out.println("<table  height='22' border='0' cellpadding='0' cellspacing='0' align='center'>");
out.println("<tr>");

out.println("<td class='boldEleven'>");
out.println("<select name='allSelect' class='formText135' id='allSelect'  style='width:200' onchange=\"mySelect()\">");
for(int ht=0; ht<transactiontypedata.length;ht++)
	out.println("<option value='"+transactiontypedata[ht][0]+"'>"+transactiontypedata[ht][1]+"</option>");
out.println("</select>");
out.println("</td>");
out.println("<td width='60' class='boldEleven'>mySelect</td>");

out.println("<td class='boldEleven'> <input id='Astatus' name='Astatus' type='checkbox' value='A' onClick=\"AllSelects()\" ></td>");
out.println("<td class='boldEleven'>Accept all</td>");
...
if(data.length>0)
{
	...
	out.println("<td class='boldEleven' align='center'><b>Transaction </b>");
	...
	for(int u=0;u<data.length;u++)
	{
		...
		out.println("<td class='boldEleven'> ");
		out.println("<select name='transactiontype"+u+"' class='formText135' id='transactiontype"+u+"'  style='width:200'>");
	   ...
	   for(int tt=0; tt<transactiontypedata.length;tt++)
		...
		/*out.println("<option value='1'>Receipts</option>");
		out.println("<option value='2'>TDS</option>");
		out.println("<option value='3'>Freight</option>");*/
		out.println("</select>");
		out.println("</td>");
		...
		out.println("onKeyPress=\"return blockNonNumbers(this, event, true, true);\"  > <div id='Valid"+(u)+"'></div>	");
		out.println("</td>");
...
if(data.length>0)
{
	%>                                    
					  ...
					  %>
									...
									type="text" class="formText135"   onBlur="upperMe(this)"    id="Challanno" size="27" maxlength="50"></td>
						              </tr>
							          <tr  id="chk3">
							            <td class="boldEleven">Cheque/ D.D  / Date   [ <span class="headerBackground">Bank Reflect Date</span> ]</td>
						...
						//-->
									...
									class="bolddeepred">* </span></td>
							            <td><input name="chequeamount" size="27" type="text" class="formText135" id="chequeamount" maxlength="15" onKeyUp="extractNumber(this,2,true) " onKeyPress="return blockNonNumbers(this, event, true, true);"
									   ></td>
						              </tr>
							          <tr>
...
function checkfillAmountAndChequeAmount()
{
	try
	{
		...
		var difference =  Math.abs(chequeamount - sum);
        console.log("difference::"+difference);
		if( difference >=100){
			alert("chequeamount:"+chequeamount +" sum:"+sum +" difference:"+difference );
			if( chequeamount != sum )
			{
...
function mySelect() {
  var x = document.getElementById("allSelect").value;
  var len=document.getElementById("totalrecords").value;
	for(var i=0;i<len;i++)
	{
		var fline="transactiontype"+i;
		 setOptionValue(fline,x) ;
	}
