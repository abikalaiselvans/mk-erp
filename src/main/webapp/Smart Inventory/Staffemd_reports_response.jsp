...
try
{
		...
		sql = sql+ " if(a.CHR_STATUS='Y','Returned','Not Return') ,INT_EMDID ";
		...
		if(data.length>0)
		{
			for(int u=0;u<data.length;u++)
			{
				...
				child.addElement("<a target='_blank' href='EMDPrint.jsp?Rowid="+data[u][19]+"' >Print</a>");
				mn.add(child); 
			}
		} 
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="Print" media="html"   sortable="true"><%=temp.elementAt(20)%></display:column>
					    
					<display:setProperty name="export.excel.filename" value="emd_reports.xls"/>
					<display:setProperty name="export.pdf.filename" value="emd_reports.pdf"/>
					<display:setProperty name="export.csv.filename" value="emd_reports.csv"/>
					<display:setProperty name="export.pdf"  value="true" />
					 
					 
</display:table> 
    
<br />	<center>	<a href='emd_reports.jsp'> CLOSE</a></center>
 <%
}
catch(Exception e)
{
