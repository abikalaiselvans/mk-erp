	  	...
	  	try
			{
				...
				if("Direct".equals(Direct))
				{
					...
					sql = sql+"  ,FIND_A_EMPLOYEE_NAME_ONLY(a.CHR_PAYEMENTCOLLECTEDBY)  ,a.CHR_PAYEMENTCOLLECTEDBY  ";//extra
					sql = sql + " ,e.CHR_COMPANYNAME,f.CHR_BRANCHNAME,datediff(a.CHR_CHKDATE,d.DAT_SALESDATE),g.CHR_DIVICODE, h.CHR_TRANSACTIONTYPE ";
					sql = sql + " FROM inv_t_customersalespayment a,";
					sql = sql+" inv_m_customerinfo b ,inv_t_directsales d ,com_m_company e, com_m_branch f,inv_m_division g , inv_m_transactiontype h ";
					sql = sql+" WHERE a.INT_CUSTOMERID = b.INT_CUSTOMERID ";
					sql = sql+"   AND a.CHR_SALESORDERNO =d.CHR_SALESNO AND d.INT_BRANCHID = f.INT_BRANCHID AND f.INT_COMPANYID =e.INT_COMPANYID  ";
					sql = sql + "  AND d.INT_DIVIID = g.INT_DIVIID   AND a.INT_TRANSACTIONTYPE = h.INT_TRANSACTIONTYPE ";
					if(!"0".equals(transactiontype))
						sql = sql+" AND a.INT_TRANSACTIONTYPE ="+transactiontype;
					if(!"0".equals(Branch))
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="Transaction" sortable="true"><%=temp.elementAt(16)%></display:column>
