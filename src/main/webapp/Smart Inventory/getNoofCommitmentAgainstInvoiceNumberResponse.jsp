...
<%
 	...
 	String reportheader="NO OF COMMITMENT AGAINT INVOICE NUMBER ";
	...
	if(serial.length >0)
	{
		sql = " SELECT a.CHR_SALESNO,DATE_FORMAT(a.DAT_SALESDATE,'%d-%b-%Y'),COUNT(*),FUN_INV_DIVISION(a.INT_DIVIID),  ";
		sql = sql + " FIND_A_CUSTOMER_NAME(a.INT_CUSTOMERID),a.DOU_TOTALAMOUNT amount,FUN_GET_PAIDAMOUNT(a.CHR_SALESNO) paidamount,  ";
		sql = sql + " (a.DOU_TOTALAMOUNT-FUN_GET_PAIDAMOUNT(a.CHR_SALESNO) ) balance  ";
		sql = sql + " FROM inv_t_directsales a, inv_t_customersalespayment b  ";
		sql = sql + " WHERE a.CHR_SALESNO =b.CHR_SALESORDERNO   ";
		sql = sql + " AND   a.CHR_SALESNO IN ( "+serials+" '0') ";
		sql = sql + " GROUP BY  b.CHR_SALESORDERNO  ";
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
	...
	<display:column title="COUNT "  style="text-align:right" media="html" sortable="true"><%=temp.elementAt(3)%></display:column>
	<display:column title="DIVISION " sortable="true"><%=temp.elementAt(4)%></display:column>
	<display:column title="CUSTOMER " sortable="true"><%=temp.elementAt(5)%></display:column>
	<display:column title="NETAMOUNT " style="text-align:right"  sortable="true"><%=temp.elementAt(6)%></display:column>
	<display:column title="PAIDAMOUNT "  style="text-align:right" sortable="true"><%=temp.elementAt(7)%></display:column>
