...
{
String sql = "";
sql = " SELECT  c.CHR_NAME ,  YEAR(a.DAT_START),   ";  
sql = sql + " (MONTH(a.DAT_START)-1),DAY(a.DAT_START), DATE_FORMAT(a.DAT_START,'%H'),DATE_FORMAT(a.DAT_START,'%i'),   "; 
sql = sql + " YEAR(a.DAT_START), (MONTH(a.DAT_START)-1),DAY(a.DAT_START), DATE_FORMAT(a.DAT_START,'%H'),DATE_FORMAT(a.DAT_START,'%i') , ";   
//sql = sql + " YEAR(a.DAT_END), (MONTH(a.DAT_END)-1),DAY(a.DAT_END), DATE_FORMAT(a.DAT_END,'%H'),DATE_FORMAT(a.DAT_END,'%i') , ";   

sql = sql + "  a.CHR_DESC     ";
sql = sql + " FROM mkt_t_daillycalls  a , mkt_m_customerinfo b,mkt_m_customername c   WHERE a.INT_CUSTOMERID = b.INT_CUSTOMERID ";
sql = sql + " AND b.INT_CUSTOMERNAMEID = c.INT_CUSTOMERNAMEID AND  a.CHR_STATUS = 'N' AND a.CHR_EMPID='"+session.getAttribute("EMPID")+"' ";
//out.println(sql);
		...
		//newWindow.focus( );
			...
			events: [
				 ...
				 if(Data.length>0)
				{
					...
					{
						out.println(" {");
						out.println(" title: '"+ Data[u][0]+"' ,");
						out.println(" start: new Date("+Data[u][1]+", "+Data[u][2]+", "+Data[u][3]+","+Data[u][4]+","+Data[u][5]+"),");
						 out.println(" end: new Date("+Data[u][6]+", "+Data[u][7]+", "+Data[u][8]+","+Data[u][9]+","+Data[u][10]+"),"); 
...
out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;CALLS VIEW</h3></span>");
%>
</center>
 <div id='calendar'></div>
</body>
