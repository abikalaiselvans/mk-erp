...
todate = DateUtil.FormateDateSys(todate);
		  ...
		  String sql = "";
		  sql = " SELECT  CONCAT(c.CHR_CALLTYPE,'/',SUM(DOU_VALUE)), SUM(DOU_VALUE) FROM mkt_t_daillycalls a, mkt_k_calltype c ";
		  ...
		  sql = sql+ " GROUP BY   a.CHR_CALLTYPE ORDER BY  c.CHR_CALLTYPE  ";
			...
			sql = "SELECT FIND_A_EMPLOYEE_ID_NAME('"+ename+"')";
			for(int u=0;u<datas.length;u++)
				data.setValue(datas[u][0], new Double(datas[u][1]));	
			
			JFreeChart chart = ChartFactory.createPieChart("Pie Chart with value for  "+CommonFunctions.QueryExecute(sql)[0][0], data, true, true, false);
			try 
			{
		...
		<td><table width="70" border="0" align="center" cellpadding="1"
          ...
          <tr>
           <td width="56"><input type="Button" class="ButtonHead" 
					name="submit"  value="Close"   accesskey="c"  onClick="redirect('DailyCallsEmployeeWiseGraphwithValue.jsp')" /></td>
          </tr>
        </table></td>
	</tr>
</table>
