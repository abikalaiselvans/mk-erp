...
function itemproductpriceMessages() 
{
	...
	for(loop = 0; loop < batchs.childNodes.length; loop++) 
    {
			...
			var tb=document.getElementById("odiv"+idposition);
  	 		tb.innerHTML="<font color='red'>"+document.getElementById("desc"+idposition).value+"</font>";
	...
	function insRow()
	{
		...
		var vf5 = "onKeyPress=\" return numeric_only(event,'"+s7+"','15') \"";
		 ...
		 x.insertCell(1).innerHTML="<div id='"+div+"'></div>";
	  	 ...
	  	 x.insertCell(2).innerHTML="<textarea name='"+s2+"' cols='10' rows='5' class='formText135' id='"+s2+"'  onKeyPress=\"textArea('"+s2+"','450')\"></textarea><div id='"+div1+"'></div>";
		x.insertCell(3).innerHTML="<input type='text' size=10 value='1'    onblur='calculateValue(this)' title='Quantity' class='formText135'  name='"+s3+"' id ='"+s3+"' " +vf1+" style='text-align:right' maxlength='3' />";
				...
				<tr>
					...
					<table width="800" border="0" align="center" cellpadding="1"
						...
						<tr>
							<td class="bold1" > 
                            <center>
                            QUOTATION CONVERT TO CPO
						      <%
String quotationnumber=request.getParameter("quotationnumber");
sql="SELECT a.INT_QUOTATIONID,a.CHR_QUOTATIONNO,a.INT_CUSTOMERID,DATE_FORMAT(a.DAT_QUOTATIONDATE,'%d-%m-%Y')";
sql = sql + "  ,DATE_FORMAT(a.DAT_REQUESTDATE,'%d-%m-%Y'),a.CHR_STATUS,a.INT_CONDITIONID,";
sql = sql + "  a.CHR_DES,a.CHR_REF,a.INT_TAXID,a.DOU_DISCOUNT,a.DOU_TAXAMOUNT,";
sql = sql + "  a.DOU_TOTALAMOUNT,a.CHR_STATUS,a.CHR_ADDRESSID1,a.CHR_ADDRESSID2 ,a.DOU_AMOUNT ,a.INT_DIVIID, ";
sql = sql + "  a.CHR_CONTACTDETAILS, a.CHR_CONTACTNO, ";
sql = sql + "  FIND_A_MKT_CUSTOMER_NAME(a.INT_CUSTOMERID),a.CHR_ADDRESSFLAG,FIND_A_MKT_CUSTOMER_ADDRESS(a.INT_CUSTOMERID), ";
sql = sql + "  a.INT_OFFICEID ,b.INT_INV_INVENTORY_ID from  mkt_t_quotation a, mkt_m_customerinfo b  where a.INT_CUSTOMERID = b.INT_CUSTOMERID ";
sql = sql + "   AND a.CHR_QUOTATIONNO='"+quotationnumber+"'";
//out.println(sql);
...
%>
						...
						<tr>
							...
							<table width="800" border="0" align="center" cellpadding="1"
								...
								<tr>
									...
									<div align="left"><%=purchaseInfo[0][1]%><input name="sfno" type="hidden" value="<%=purchaseInfo[0][1]%>"></div>									</td>
			...
			// 
 			%> 
<select name="customerId" id="customerId" class="formText135"  style="width:170"  onchange="loadAddress('customerId','order') ">
...
%>
							      ...
							      </div><span class="errormessage"><%=purchaseInfo[0][20]%></span>
                                    </td>
								  ...
								  <div align="left">Order From<span class="ui-state-error-text"> * </span></div>									</td>
									...
									<div align="left"><textarea name="order" cols="25"
										rows="5"  onKeyPress="textArea('order','450')" class="formText135" id="order" tabindex="9"></textarea>
								    <span class="errormessage"><%=purchaseInfo[0][14]%></span> </div>									</td>
								</tr>
								<tr>
									<td valign="top" class="boldEleven">Office   <span class="ui-state-error-text"> * </span></td>
...
<script language="javascript">setOptionValue('office','<%=purchaseInfo[0][23]%>')</script>  
<script language="javascript">setOptionValue('customerId','<%=purchaseInfo[0][24]%>')</script>                                    
								...
								<tr>
							      ...
							      <td rowspan="3" align="left" valign="top" class="boldEleven">
									<div align="left" class="errormessage"><textarea name="ship" cols="25"
										rows="5"  onKeyPress="textArea('ship','450')" class="formText135" id="ship" tabindex="9"></textarea><%=purchaseInfo[0][15]%>
								  </div></td>
							  </tr>
								<tr>
						...
						<tr>
							<td>
							 <table width="100%" border="1" cellpadding="1" cellspacing="2"	id="myTable" bordercolor="#000000">
								<tr><th class="boldEleven"><b>Type</b></th>
							...
							<table width="100%" border="0" align="center" cellpadding="1"
							 ...
							 %>
								...
								<tr>
								    ...
								    <input name="filename"
										type="hidden" value="QuoteConversion" /> 
								    <input name="actionS"
										type="hidden" value="MKTQuoteConversionAdd" /> 
								    <input name="param"
