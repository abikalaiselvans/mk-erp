...
//out.println(sql);
	...
	String bsql="SELECT INT_PFAMT,INT_ESIAMT,INT_CPY_PFAMT,INT_CPY_PENSIONAMT,INT_CPY_ESIAMT,INT_PFLIMITMINIMUM ,  INT_PFLIMIT,INT_ESILIMIT FROM m_institution WHERE INT_ID=1";
		...
		for(int u=0;u<Data.length;u++)
		{
			...
			if("C".equals(Data[u][38]))
				child.addElement("COMMON");
			else if("Y".equals(Data[u][38]))
				child.addElement("YES");
			else if("N".equals(Data[u][38]))
				child.addElement("NO");
			...
			if("C".equals(Data[u][40]))
				child.addElement("COMMON");
			else if("Y".equals(Data[u][40]))
				child.addElement("YES");
			else if("N".equals(Data[u][40]))
				child.addElement("NO");
			...
			child.addElement(Data[u][55] +"-Yrs");	 
			mn.add(child);
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					%>
					<display:column title="SUM (gross allowance)" sortable="true"><%=temp.elementAt(col+1)%></display:column>
					<display:column title="SUM REIMBURSEMENT" sortable="true"><%=temp.elementAt(col+2)%></display:column>
					...
					<%
					 ...
					 col=10+allowancefield.length+3+recoveryfield.length+1;
					%>
					<display:column title="Revoery sum" sortable="true"><%=temp.elementAt(col)%></display:column>
					
					<display:column title="PF Type" sortable="true"><%=temp.elementAt(col+1)%></display:column>
					<display:column title="PF Number" sortable="true"><%=temp.elementAt(col+2)%></display:column>
					<display:column title="PF AMOUNT" sortable="true"><%=temp.elementAt(col+3)%></display:column>
					
					<display:column title="ESI Type" sortable="true"><%=temp.elementAt(col+4)%></display:column>
					<display:column title="ESI Number" sortable="true"><%=temp.elementAt(col+5)%></display:column>
					<display:column title="ESI AMOUNT" sortable="true"><%=temp.elementAt(col+6)%></display:column>
					
					<display:column title="Employer PensionFund" sortable="true"><%=temp.elementAt(col+7)%></display:column>
					<display:column title="Employer PF" sortable="true"><%=temp.elementAt(col+8)%></display:column>
					<display:column title="Employer ESI" sortable="true"><%=temp.elementAt(col+9)%></display:column>
					
					<display:column title="Total Salary" sortable="true"><%=temp.elementAt(col+10)%></display:column>
					<display:column title="CTC" sortable="true"><%=temp.elementAt(col+11)%></display:column>
					
		 			<display:column title="GENDER" sortable="true"><%=temp.elementAt(col+12)%></display:column>
					<display:column title="DOB" sortable="true"><%=temp.elementAt(col+13)%></display:column>
					<display:column title="DOJ" sortable="true"><%=temp.elementAt(col+14)%></display:column>
					<display:column title="MARTIAL STATUS" sortable="true"><%=temp.elementAt(col+15)%></display:column>
					<display:column title="WEDDING DATE" sortable="true"><%=temp.elementAt(col+16)%></display:column>
					<display:column title="QUALIFICATION" sortable="true"><%=temp.elementAt(col+17)%></display:column>
					<display:column title="BLOOD" sortable="true"><%=temp.elementAt(col+18)%></display:column>
					<display:column title="Permanent Address " sortable="true"><%=temp.elementAt(col+19)%></display:column>
					<display:column title="Communication Address" sortable="true"><%=temp.elementAt(col+20)%></display:column>
					<display:column title="MAIL" sortable="true"><%=temp.elementAt(col+21)%></display:column>
					<display:column title="MOBILE" sortable="true"><%=temp.elementAt(col+22)%></display:column>
					<display:column title="BANK" sortable="true"><%=temp.elementAt(col+23)%></display:column>
					<display:column title="A/C Number" sortable="true"><%=temp.elementAt(col+24)%></display:column>
					<display:column title="Year Of Experience" sortable="true"><%=temp.elementAt(col+25)%></display:column>
