...
sql=sql+" AND  a.CHR_EMPID = g.CHR_EMPID   AND  g.INT_APPRAISALID =  "+appraisalid +" AND  a.CHR_EMPID = '"+Staffid+"' ";

sql=sql+" ORDER BY a.CHR_STAFFNAME "; 
//out.println(sql);
String datas[][] =  CommonFunctions.QueryExecute(sql);
sql = " SELECT CONCAT('Type - ',CHR_PFCALCULATIONTYPE) FROM pay_t_appraisal WHERE INT_APPRAISALID="+appraisalid ;%>
							</div>							</td>
						</tr>
						<tr>
							<td colspan="2">
						...
						<tr>
						  <td height="19" colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                              <td width="22%" class="bold1">Emp id</td>
                              <td width="33%" class="boldEleven"><%=datas[0][0]%></td>
...
sql = sql +  " AND  a.INT_APPRAISALID =  "+appraisalid;
sql = sql +  " AND  b.INT_APPRAISALID =  "+appraisalid;
sql = sql +  " AND  c.INT_APPRAISALID =  "+appraisalid;
sql = sql +  " AND  d.INT_APPRAISALID =  "+appraisalid;
	...
	if(!"-".equals(takeupdatas[0][0]))
	{
		...
		out.println("</center>");
						...
						<tr>
							...
							<input name="actionS" type="hidden" id="actionS" value="PAYPayRollRevisionNext" /> 
