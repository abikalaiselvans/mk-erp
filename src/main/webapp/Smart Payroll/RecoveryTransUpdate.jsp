...
try{
	...
	{
		st.execute("INSERT INTO pay_t_salaryrecovery(CHR_EMPID,CHR_MONTH,INT_YEAR) VALUES('"+empId+"','"+month+"',"+year+")");
	}
	
	String sqlUp="UPDATE pay_t_salaryrecovery set CHR_EMPID='"+empId+"'";
	String all=null;
	int allowance=0;
	for(int j=1;j<=7;j++)
	{
	  all=""+request.getParameter("R00"+j);
	  if(all.equals("null")|| all.equals(""))
		  sqlUp=sqlUp+",R00"+j+"=0";
  	  else{
  		allowance=allowance+Integer.parseInt(all);
  		sqlUp=sqlUp+",R00"+j+"="+all;
  	  }
	}
	...
	st.execute(sqlUp);
	sqlUp="UPDATE pay_t_nsalarydetailspf SET INT_RECOVERY="+allowance;
	sqlUp=sqlUp+" WHERE CHR_EMPID='"+empId+"' AND CHR_MONTH='"+month+"' AND INT_YEAR="+year;
}
catch(Exception e){
