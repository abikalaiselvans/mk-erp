...
<%
	...
	String holdsql = "SELECT CHR_EMPID FROM pay_t_temporarysuspension WHERE CHR_MONTH='"+repmon+"' AND INT_YEAR="+year;
	System.out.println(holdsql);
	String holdsqldata[][] = CommonFunctions.QueryExecute(holdsql);
	String holdempid="";
	if(holdsqldata.length>0)
	{
		for(int u=0;u<holdsqldata.length;u++)
			holdempid = holdempid+ "'"+holdsqldata[u][0]+"'  ,";
		holdempid=holdempid.trim();			
		holdempid = holdempid.substring(0,holdempid.length()-1);	
	}	
	...
	sql = sql+ "  WHERE a.CHR_MONTH ='"+repmon+"'  " ;
	...
	if(holdsqldata.length>0)
			sql = sql+ "  AND a.CHR_EMPID IN ( "+holdempid+")"; 
