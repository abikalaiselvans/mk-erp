...
<body  onpaste='return false;'>
	...
	if(!branchId.equals("0"))
     ...
     for(int u=0; u<data.length;u++)
	 {
			...
			for(int v=0;v<recovery.length;v++)
	...
	out.println("<br><center><h3> RECOVERY DETAILS FOR THE MONTH  ::  "+mon+" AND YEAR ::  "+year+" </h3></center><br>");
	 ...
	 <display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="Year" sortable="true"><%=temp.elementAt(6)%></display:column>
					 ...
					 %>
					 	...
					 	<display:column title="<%=recovery[v][0]%>" sortable="true"><%=temp.elementAt(v+7)%></display:column>
					 <%
					 }
				     %>
					...
					<display:setProperty name="export.csv.filename" value="RecMonthWise.csv"/>
					<display:setProperty name="export.excel.filename" value="RecMonthWise.xls"/>
					<display:setProperty name="export.xml.filename" value="RecMonthWise.xml"/>
					<display:setProperty name="export.pdf.filename" value="RecMonthWise.pdf"/>
					<display:setProperty name="export.pdf" value="true" />
					 
</display:table>   
 
 <br />	<center>	<a href='RecMonthWise.jsp'> CLOSE</a></center>
