...
try
{
	...
	String off="";
	String sql="";
	sql = " SELECT  a.CHR_EMPID,b.CHR_STAFFNAME,c.CHR_OFFICENAME ,a.CHR_MONTH,a.INT_YEAR ";  
	sql = sql + " FROM pay_t_temporarysuspension a, com_m_staff b, com_m_office c ";
	...
	//out.println(sql);
	String reportdata[][] = CommonFunctions.QueryExecute(sql);
	...
	out.println("<br><center><h3> SALARY HOLDING LIST  ("+repmon+"-"+Year+" )  </h3></center><br>");
...
<display:table   id="_table" name="table"   export="true" pagesize="25">
					...
					<display:column title="Month"sortable="true"><%=temp.elementAt(4)%></display:column>
					<display:column title="Year" sortable="true"><%=temp.elementAt(5)%></display:column>
					  
					
					<display:setProperty name="export.csv.filename" value="Rept_SalaryHold.csv"/>
					<display:setProperty name="export.excel.filename" value="Rept_SalaryHold.xls"/>
					<display:setProperty name="export.xml.filename" value="Rept_SalaryHold.xml"/>
					<display:setProperty name="export.pdf.filename" value="Rept_SalaryHold.pdf"/>
					<display:setProperty name="export.pdf" value="true" />
					 
</display:table>
	
     
<br />	<center>	<a href='Rept_SalaryHold.jsp'> CLOSE</a></center>
