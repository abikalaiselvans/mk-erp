...
try
{
	...
	for(int k=0;k<recoveryData.length;k++)
	{
	 ...
	 sql = " SELECT c.CHR_OFFICENAME,d.CHR_DEPARTNAME,e.CHR_CATEGORYNAME,SUM(b.DOU_BASIC), ";
	 
	 //sql = sql + " SUM(b.A001),SUM(b.A002),SUM(b.A003),SUM(b.A004),SUM(b.A005),SUM(b.A006), ";
	 //sql = sql + " SUM(b.A007),SUM(b.A008),SUM(b.A009),SUM(b.A0010),SUM(b.A0011),SUM(b.A0012),SUM(b.A0013),SUM(b.A0014),SUM(b.A0015), ";
	 sql = sql + allowancefield ;
	 sql = sql + " SUM(b.DOU_ATOTAL1),SUM(b.DOU_ATOTAL2),SUM(b.DOU_GROSSPAY), ";
	 
	 //sql = sql + " SUM(b.R001),SUM(b.R002),SUM(b.R003),SUM(b.R004),SUM(b.R005),SUM(b.R006),SUM(b.R007),SUM(b.R008), ";
	 ...
	 sql = sql + " GROUP BY  a.INT_OFFICEID,a.INT_DEPARTID,a.CHR_CATEGORY ";  //a.CHR_EMPID,
	 sql = sql + " ORDER BY c.CHR_OFFICENAME,d.CHR_DEPARTNAME,e.CHR_CATEGORYNAME ";
	 //out.println(sql);
	 int totalfield = 4+  AllowanceData.length+3+recoveryData.length+23;
	 String data[][] =com.my.org.erp.common.CommonFunctions.QueryExecute(sql);
     
	 String headfield="";
	 headfield ="S.NO,OFFICE,DEPRT,CATEGORY,BASIC,";
	 ...
	 { 
		 for(int u=0; u<data.length;u++)
		 {
				...
				for(int v=0; v<totalfield;v++)
					if(v==0)
						child.addElement("<a target='_blank' href='SalaryConsolidationDepartmentwiseResponse.jsp?company="+company+"&branch="+branch+"&Office="+Office+"&Month="+Month+"&Year="+Year+"&Depart="+data[u][1]+"&Category="+data[u][2]+"'>"+data[u][0]+"</a>  ");
					else		
		 ...
		 //SUM 
		 sql = " SELECT c.CHR_OFFICENAME,' ',' TOTAL',SUM(b.DOU_BASIC), ";
		 sql = sql + allowancefield ;
		 sql = sql + " SUM(b.DOU_ATOTAL1),SUM(b.DOU_ATOTAL2),SUM(b.DOU_GROSSPAY), ";
		 sql = sql + recoveryfield ;
		 sql = sql + " SUM(RTOTAL),SUM(DOU_PF),SUM(DOU_ESI),SUM(DOU_MESS),SUM(DOU_ADVANCE),SUM(DOU_TOTALRECOVERY),SUM(DOU_NETSAL),SUM(DOU_MONTHSAL), ";
		 ...
		 if(!"-1".equals(branch))
			sql = sql + "  AND a.INT_BRANCHID="+branch;
		 sql = sql + " GROUP BY  a.INT_OFFICEID  ";   
		 sql = sql + " ORDER BY c.CHR_OFFICENAME,d.CHR_DEPARTNAME,e.CHR_CATEGORYNAME ";
...
<br />	<center>	<a href='SalaryConsolidation.jsp'> CLOSE</a></center><br /><br />
...
<%
	...
	{ 
		...
		int totalcoulmn = AllowanceData.length+recoveryData.length+30;
		for(int x=0;x<reportdata.length;x++)
		{ 
			out.println("<TR>");
			out.println("<Td bgColor=#ffffff class='boldEleven'>"+(x+1) +"</Td>");
			out.println("<Td bgColor=#ffffff class='boldEleven'><a target='_blank' href='SalaryConsolidationDepartmentwiseResponse.jsp?company="+company+"&branch="+branch+"&Office="+Office+"&Month="+Month+"&Year="+Year+"&Depart="+reportdata[x][1]+"&Category="+reportdata[x][2]+"'>"+reportdata[x][0]+"</a></Td>");
			for(int k=1;k<totalcoulmn;k++)
