...
<%
	...
	addsql=addsql+" IF( PAY_FUN_GET_TERMINATE(a.CHR_EMPID ),CONCAT(a.CHR_EMPID,'****'),IF(PAY_FUN_GET_TEMPORARYSUSPENSION(a.CHR_EMPID )=1,CONCAT(a.CHR_EMPID,'####'),a.CHR_EMPID)), ";
	//IF(PAY_FUN_GET_TEMPORARYSUSPENSION(a.CHR_EMPID)=1, CONCAT(a.CHR_EMPID,'###'),a.CHR_EMPID) , 
	addsql=addsql+" a.CHR_EMPNAME,e.CHR_COMPANYNAME , z1.CHR_BRANCHNAME,d.CHR_OFFICENAME,c.CHR_DEPARTNAME,z.CHR_CATEGORYNAME,b.CHR_TYPE,";
	...
	if(!"null".equals(""+request.getParameter("Acheckbox")))
	{
		...
		for(int u=0;u<Allowance.length;u++)
		{
			...
			String alldata[][]= CommonFunctions.QueryExecute(allsql);
	...
	if("Y".equals(acno))
	{
		addsql=addsql+" CONCAT('A/c:',TRIM(b.CHR_ACCNO) )  , ";
	...
	if(!"0".equals(office))
	{
		...
		offname= CommonFunction.stringGetAnySelectField(sql, "CHR_OFFICENAME");
	...
	String reportdata[][] =  CommonFunction.RecordSetArray(sql);
	...
	{
		out.println("<tr  bgcolor='#ffffff'><td colspan=\""+repdata.length+"\"  align='center'><h3><center>"+repName+"</center></h3></td></tr>");
		...
		for(int x=0;x<reportdata.length;x++)
		{
			...
			*/
			if(staffid.endsWith("####") || staffid.endsWith("****")  )
			{
				out.println("<tr bgcolor='#ff0000'>");
				out.println("<td  class=\"boldEleven\">"+(x+1));
			}
