...
<script language="javascript" src="../JavaScript/comfunction.js"> </script><body  onpaste='return false;'><br><br><br><br>
	...
	<%
		...
		String sql=" SELECT  e.CHR_OFFICENAME,a.CHR_EMPID,b.CHR_STAFFNAME, c.CHR_REMINDERNAME,a.CHR_POLICYNO, a.CHT_PERIOD , a.DOU_PREMIUM,DATE_FORMAT(a.DT_DATE,'%d-%m-%Y'),   DATE_FORMAT(a.DT_DUEDATE,'%d-%m-%Y'), DATE_FORMAT(a.DT_MATURITY,'%d-%m-%Y'),  a.DOU_SUMASSURED,a.CHR_REGNUMBER, a.CHR_MAKE,a.CHR_MODEL,d.CHR_ISSUERNAME,   DATE_FORMAT(a.DT_VALIDUPTO,'%d-%m-%Y'),	a.CHR_STATUS,a.CHR_VECHICLESTATUS   FROM veh_t_insurancereminder a  ,com_m_staff b,vehicle_m_insurancereminder c,vehicle_m_insuranceissuer d,   	com_m_office e   WHERE a.CHR_EMPID=b.CHR_EMPID  AND a.INT_REMINDERID = c.INT_REMINDERID AND a.INT_ISSUERID=d.INT_ISSUERID AND a.INT_BRANCHID=e.INT_OFFICEID  ";
		
		if(!"0".equals(office))
			sql = sql +" AND e.INT_OFFICEID = "+office;
		if(!"0".equals(reminder))
			sql = sql +" AND a.INT_REMINDERID ="+reminder;
		if(!"0".equals(Issuer))
			sql = sql +" AND a.INT_ISSUERID = "+Issuer;
		if(!"0".equals(pstatus))
			sql = sql +" AND a.CHR_STATUS ='"+pstatus+"'";
		if(!"0".equals(vstatus))
			sql = sql +" AND a.CHR_VECHICLESTATUS ='"+vstatus+"'";	
			
		sql = sql + " AND a.DT_DUEDATE >='"+frmdate+"'";
		sql = sql + " AND a.DT_DUEDATE <='"+todate+"'";
			
		sql = sql + " ORDER BY e.CHR_OFFICENAME,a.DT_DUEDATE ";
