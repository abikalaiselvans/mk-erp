...
$('#Add').unbind("click").on("click",function() 
	{
		...
		$("#description").val("");
		$("#erowid").val("0");
		showActionModal("AddEquipementMake","Add");
});
...
$('#Edit').unbind("click").on("click",function() 
	...
	else if(countSelectedCheckLength == 0) 
	{
  	...
  	else 
  	{
  		...
  		callAjaxMethod("POST","editEquipementMake",param,bindEditValues);
   }
});
...
function bindEditValues(result) 
{
	try
	{
		...
		showActionModal("UpdateEquipementMake","Update");
...
function loadEquipementMakeStartwith(letter) 
{
	var param = {"startLetter":letter };
	callAjaxMethod("POST","sortEquipmentMake",param,equipmentMakeMessage);
}
/* Success method for startLetter as well as to replace the sorted content on table */   	
function equipmentMakeMessage(result) 
{
	try
	{
		var rowLength =  result.length;
		var content="";
	   	var id,name,typename;
	   	if(rowLength === 0) 
		{
...
function DeleteProceed()
{
	try
	{
		...
		callAjaxMethod("POST","deleteEquipmentMake",param,bindModifiedList);	
	}
	catch(err)
	{
		alert(err);
...
function bindModifiedList(result) 
{
	if(result == "success") 
	{
		window.location.href = "EquipmentMake";
   	}
}
