...
$('#Add').unbind("click").on("click",function() 
	{
		...
		$("#edescription").val("");
		$("#erowid").val("0");
		showActionModal("AddGeneralGroup","Add");
});
...
$('#Edit').unbind("click").on("click",function() 
	...
	else if(countSelectedCheckLength == 0) 
	{
  	...
  	else 
  	{
  		...
  		callAjaxMethod("POST","editGeneralGroup",param,bindEditValues);
   }
});
...
function bindEditValues(result) 
{
	try
	{
		...
		showActionModal("UpdateGeneralGroup","Update");
...
function loadGeneralGroup(letter) 
{
	var param = {"startLetter":letter };
	callAjaxMethod("POST","sortEquipmentMake",param,loadGeneralGroupMessage);
}
/* Success method for startLetter as well as to replace the sorted content on table */   	
function loadGeneralGroupMessage(result) 
{
	try
	{
...
function DeleteProceed()
{
	try
	{
		...
		callAjaxMethod("POST","deleteGeneralGroup",param,bindModifiedList);	
	}
	catch(err)
	{
		alert(err);
...
function bindModifiedList(result) 
{
	if(result == "success") 
	{
		window.location.href = "GeneralGroup";
   	}
}
