...
$('#Add').unbind("click").on("click",function() 
	{
		...
		$("#description").val("");
		$("#erowid").val("0");
		showActionModal("AddProblemGroup","Add");
});
...
$('#Edit').unbind("click").on("click",function() 
	...
	else if(countSelectedCheckLength == 0) 
	{
  	...
  	else 
  	{
  		...
  		callAjaxMethod("POST","editProblemGroup",param,bindEditValues);
   }
});
...
function bindEditValues(result) 
{
	try
	{
		...
		showActionModal("UpdateProblemGroup","Update");
...
function loadProblemGroup(letter)  
{
	var param = {"startLetter":letter };
	callAjaxMethod("POST","sortProblemGroup",param,equipmentMakeMessage);
}
...
function DeleteProceed()
{
	try
	{
		...
		callAjaxMethod("POST","deleteProblemGroup",param,bindModifiedList);	
	}
	catch(err)
	{
		alert(err);
...
function bindModifiedList(result) 
{
	if(result == "success") 
	{
		window.location.href = "ProblemGroup";
   	}
}
