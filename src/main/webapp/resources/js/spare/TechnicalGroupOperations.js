...
$('#Add').unbind("click").on("click",function() 
	{
		...
		$("#description").val("");
		$("#erowid").val("0");
		showActionModal("AddTechnicalGroup","Add");
});
...
$('#Edit').unbind("click").on("click",function() 
	...
	else if(countSelectedCheckLength == 0) 
	{
  	...
  	else 
  	{
  		...
  		callAjaxMethod("POST","editTechnicalGroup",param,bindEditValues);
   }
});
...
function bindEditValues(result) 
{
	try
	{
		...
		showActionModal("UpdateTechnicalGroup","Update");
...
function loadTechnicalGroup(letter)  
{
	var param = {"startLetter":letter };
	callAjaxMethod("POST","sortTechnicalGroup",param,TechnicalGroupMessage);
}
/* Success method for startLetter as well as to replace the sorted content on table */   	
function TechnicalGroupMessage(result) 
{
	try
	{
...
function DeleteProceed()
{
	try
	{
		...
		callAjaxMethod("POST","deleteTechnicalGroup",param,bindModifiedList);	
	}
	catch(err)
	{
		alert(err);
...
function bindModifiedList(result) 
{
	if(result == "success") 
	{
		window.location.href = "TechnicalGroup";
   	}
}
