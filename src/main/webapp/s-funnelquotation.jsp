         ...
         <section class="content">
              ...
              <div class="row">
                    <div class="col-lg-12">
                        ...
                        <%
						String sql = "";
						sql = " SELECT UPPER(MONTHNAME(NOW())),YEAR(NOW()) ";
						String dtdata[][] = CommonFunctions.QueryExecute(sql);
						%>
                       QUOTATION FOR THE MONTH OF <%=dtdata[0][0] + " - " +dtdata[0][1]%></h2>
                        <div class="table-responsive"  style="OVERFLOW:auto;width:100%;">
<%
sql = "  ( ";
sql = sql + " SELECT b.CHR_DIVICODE,(SUM(c.DOU_TOTAL)-SUM(c.DOU_TAX_AMOUNT)) ";
sql = sql + " FROM mkt_t_quotation a, inv_m_division b, mkt_t_quotationitem c ";
sql = sql + " WHERE a.INT_DIVIID =b.INT_DIVIID AND a.CHR_QUOTATIONNO =c.CHR_QUOTATIONNO  AND a.CHR_MOVING_TO_CPO = 'N'";
...
sql = sql + " UNION ";
sql = sql + " ( ";
sql = sql + " SELECT CONCAT('TOTAL'),(SUM(c.DOU_TOTAL)-SUM(c.DOU_TAX_AMOUNT))  ";
sql = sql + " FROM mkt_t_quotation a,  mkt_t_quotationitem c  ";
sql = sql + " WHERE a.CHR_QUOTATIONNO =c.CHR_QUOTATIONNO  AND a.CHR_MOVING_TO_CPO = 'N'";
sql = sql + " AND MONTHNAME(a.DAT_QUOTATIONDATE) = MONTHNAME(NOW()) ";
sql = sql + " AND YEAR(a.DAT_QUOTATIONDATE) = YEAR(NOW()) ";
sql = sql + " ) ";
...
if(edata.length>0)
{
	...
	out.println("<th align='center'>DIVISION</th>");
	...
	for(int u=0;u<edata.length;u++)
	{
		...
		out.println("<td align='right'>"+edata[u][1]+"</td>");
